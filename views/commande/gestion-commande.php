<?php
// ================================================================
// GESTION DES COMMANDES - EPENCIA SGI
// ================================================================
...
function getFactures($pdo) {
    try {
        $stmt = $pdo->query('SELECT facture_id FROM factures ORDER BY facture_id DESC');
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        return [];
    }
...
} elseif (isset($_SESSION['commandes_filters']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $search = $_SESSION['commandes_filters']['search'] ?? '';
    $filter_statut = $_SESSION['commandes_filters']['statut'] ?? '';
    $filter_produit = $_SESSION['commandes_filters']['produit_id'] ?? '';
    $filter_facture = $_SESSION['commandes_filters']['facture_id'] ?? '';
    $page = $_SESSION['commandes_filters']['page'] ?? 1;
    $offset = ($page - 1) * $limit;
}

// Construction de la requête
$sql = 'SELECT c.*, p.nom as produit_nom, p.type as produit_type, f.facture_id, u.nom_prenom as utilisateur_nom 
        FROM commandes c 
        LEFT JOIN produits p ON c.produit_id = p.produit_id 
        LEFT JOIN factures f ON c.facture_id = f.facture_id 
        LEFT JOIN utilisateurs u ON c.utilisateur_id = u.utilisateur_id';
$where = [];
$params = [];

if (!empty($search)) {
    $where[] = '(c.commande_id LIKE ? OR p.nom LIKE ?)';
    $search_term = "%$search%";
    $params[] = $search_term;
    $params[] = $search_term;
}
...
// Compter le total
$count_sql = str_replace('SELECT c.*, p.nom as produit_nom, p.type as produit_type, f.facture_id, u.nom_prenom as utilisateur_nom', 'SELECT COUNT(*)', $sql);
$count_stmt = $pdo->prepare($count_sql);
$count_stmt->execute($params);
$total_commandes = $count_stmt->fetchColumn();
$total_pages = max(1, ceil($total_commandes / $limit));

// Récupérer les commandes avec pagination
$sql .= ' ORDER BY c.commande_id DESC LIMIT ' . (int)$limit . ' OFFSET ' . (int)$offset;
...
$stmt = $pdo->query('SELECT COUNT(*) FROM commandes WHERE statut = "inactif"');
$total_inactif = $stmt->fetchColumn();

$stmt = $pdo->query('SELECT SUM(montant) as total FROM commandes WHERE statut = "actif"');
$total_montant = $stmt->fetchColumn() ?? 0;

$liste_produits = getProduits($pdo);
$liste_factures = getFactures($pdo);
$statuts_commande = ['actif', 'inactif'];
...
if (!empty($filter_produit)) {
    ...
    foreach ($liste_produits as $p) {
        if ($p['produit_id'] == $filter_produit) {
            $produit_label = $p['nom'];
            break;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'add') {
            ...
            if (empty($commande_id) || empty($produit_id) || empty($facture_id) || $quantite <= 0) {
                throw new Exception('Tous les champs obligatoires doivent être remplis.');
            }

            $stmt = $pdo->prepare('SELECT COUNT(*) FROM commandes WHERE commande_id = ?');
            $stmt->execute([$commande_id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cet ID de commande existe déjà.');
            }

            $stmt = $pdo->prepare('INSERT INTO commandes (commande_id, produit_id, facture_id, utilisateur_id, prix_initial, prix, quantite, montant, statut) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([$commande_id, $produit_id, $facture_id, $utilisateur_id, $prix_initial, $prix, $quantite, $montant, $statut]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'edit') {
            ...
            $stmt = $pdo->prepare('UPDATE commandes SET commande_id = ?, produit_id = ?, facture_id = ?, prix_initial = ?, prix = ?, quantite = ?, montant = ?, statut = ? WHERE commande_id = ?');
            $stmt->execute([$commande_id, $produit_id, $facture_id, $prix_initial, $prix, $quantite, $montant, $statut, $id]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'delete') {
            ...
            $stmt = $pdo->prepare('DELETE FROM commandes WHERE commande_id = ?');
            $stmt->execute([$id]);
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['commandes_filters']);
            header('Location: ' . $_SERVER['PHP_SELF']);
            exit;
        }
        ...
        if (isset($_POST['export_excel'])) {
            ...
            header('Content-Disposition: attachment; filename="commandes_' . date('Ymd_His') . '.xls"');
            
            $export_sql = 'SELECT c.*, p.nom as produit_nom, f.facture_id, u.nom_prenom as utilisateur_nom 
                           FROM commandes c 
                           LEFT JOIN produits p ON c.produit_id = p.produit_id 
                           LEFT JOIN factures f ON c.facture_id = f.facture_id 
                           LEFT JOIN utilisateurs u ON c.utilisateur_id = u.utilisateur_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_commande') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare('SELECT c.*, p.nom as produit_nom, f.facture_id, u.nom_prenom as utilisateur_nom 
                               FROM commandes c 
                               LEFT JOIN produits p ON c.produit_id = p.produit_id 
                               LEFT JOIN factures f ON c.facture_id = f.facture_id 
                               LEFT JOIN utilisateurs u ON c.utilisateur_id = u.utilisateur_id 
...
<head>
    ...
    <title>Gestion des Commandes - Epencia SGI</title>
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <div class="subtitle">
                ...
                Créer, modifier et suivre les commandes
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <?php if ($is_admin || $is_medecin): ?>
                <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                    <i class="bi bi-plus-circle"></i> <span>Nouvelle commande</span>
                </button>
            <?php endif; ?>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d3">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Commandes inactives</div>
            </div>
        </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID commande, produit...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Statut</label>
                <select name="statut" class="form-select">
                    <option value="">Tous les statuts</option>
                    <?php foreach ($statuts_commande as $s): ?>
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des commandes
                <span class="badge bg-secondary ms-2"><?= number_format($total_commandes) ?></span>
            </h5>
            <?php if (!empty($active_filters)): ?>
                <form method="POST" class="d-inline">
                            ...
                            <tr>
                                <td colspan="8" class="text-center py-5" style="color:var(--medical-text-muted);">
                                    ...
                                    <?php if ($total_general == 0): ?>
                                        ...
                                        <br>
                                        <span style="font-size:13px;">Cliquez sur "Nouvelle commande" pour en créer une.</span>
                                    <?php else: ?>
                                        <strong>Aucune commande trouvée avec ces filtres.</strong>
                                        <br>
                        ...
                        <?php else: ?>
                            <?php foreach ($commandes as $c): ?>
                                <tr>
                                    ...
                                    <td><strong><?= htmlspecialchars($c['produit_nom'] ?? '-') ?></strong></td>
                                    <td><?= htmlspecialchars($c['facture_id'] ?? '-') ?></td>
                                    <td><?= number_format($c['prix'], 0, ',', ' ') ?> F</td>
                                    <td class="text-center"><span class="badge bg-primary"><?= $c['quantite'] ?></span></td>
                                    <td class="fw-bold" style="color:var(--medical-teal);"><?= number_format($c['montant'], 0, ',', ' ') ?> F</td>
                                    <td class="text-center"><?= getStatusBadge($c['statut']) ?></td>
                                    <td>
                                        <div class="actions-group">
                                            <?php if ($is_admin || $is_medecin): ?>
                                                <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                                    data-id="<?= htmlspecialchars($c['commande_id']) ?>" title="Modifier">
                                                    <i class="bi bi-pencil-square"></i>
                                                </button>
                                                <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                                    data-id="<?= htmlspecialchars($c['commande_id']) ?>" 
                                                    data-produit="<?= htmlspecialchars($c['produit_nom'] ?? '-') ?>" 
                                                    title="Supprimer">
                ...
                <div class="d-flex justify-content-center p-3">
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filter_statut) ?>&produit_id=<?= urlencode($filter_produit) ?>&facture_id=<?= urlencode($filter_facture) ?>">
                                <i class="bi bi-chevron-left"></i>
                            </a>
                        </li>
                        <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filter_statut) ?>&produit_id=<?= urlencode($filter_produit) ?>&facture_id=<?= urlencode($filter_facture) ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Commande <span class="text-danger">*</span></label>
                            <input type="text" name="commande_id" class="form-control" required placeholder="Ex: CMD-001">
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="section-title"><i class="bi bi-receipt"></i> Facture</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                    ...
                    <div class="section-title"><i class="bi bi-calculator"></i> Prix et quantités</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            <label class="form-label">Prix initial</label>
                            <input type="number" step="0.01" name="prix_initial" class="form-control" placeholder="0.00">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Prix unitaire <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="prix" class="form-control" required placeholder="0.00">
                        </div>
                        <div class="col-md-4">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la commande</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette commande ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_commande_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Produit :</span>
                            <span style="font-weight:500;" id="delete_commande_produit"></span>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de la commande...</p>
            </div>
        `);
        submitBtn.hide();
        
        $.post(window.location.href, { 
            ajax_action: 'load_commande', 
            id: id 
        }, function(data) {
            if (data.success) {
                const c = data.data;
                body.html(`
                    ...
                    <input type="hidden" name="id" value="${c.commande_id}">
                    
                    <div class="section-title"><i class="bi bi-fingerprint"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Commande</label>
                            <input type="text" name="commande_id" class="form-control bg-light" value="${c.commande_id}" readonly style="font-family:monospace;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Produit <span class="text-danger">*</span></label>
                            <select name="produit_id" class="form-select" required>
                                <option value="">Sélectionnez un produit</option>
                                <?php foreach ($liste_produits as $p): ?>
                                    <option value="<?= htmlspecialchars($p['produit_id']) ?>" ${c.produit_id === '<?= $p['produit_id'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($p['nom']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            ...
                            <select name="facture_id" class="form-select" required>
                                <option value="">Sélectionnez une facture</option>
                                <?php foreach ($liste_factures as $f): ?>
                                    <option value="<?= htmlspecialchars($f['facture_id']) ?>" ${c.facture_id === '<?= $f['facture_id'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($f['facture_id']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            <label class="form-label">Prix initial</label>
                            <input type="number" step="0.01" name="prix_initial" class="form-control" value="${c.prix_initial || 0}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Prix unitaire <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="prix" class="form-control" value="${c.prix}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Quantité <span class="text-danger">*</span></label>
                            <input type="number" name="quantite" class="form-control" value="${c.quantite}" required min="1">
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Montant total <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="montant" class="form-control" value="${c.montant}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Statut</label>
                            <select name="statut" class="form-select">
                                <?php foreach ($statuts_commande as $s): ?>
                                    <option value="<?= $s ?>" ${c.statut === '<?= $s ?>' ? 'selected' : ''}>${esc('<?= ucfirst($s) ?>')}</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        $('#delete_id').val(id);
        $('#delete_commande_id').text(id);
        $('#delete_commande_produit').text(produit);
    });
});
