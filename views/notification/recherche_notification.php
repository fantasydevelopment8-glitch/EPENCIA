<?php
// ================================================================
// RECHERCHE DES NOTIFICATIONS - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_notification') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("SELECT n.*, u.nom_prenom, u.utilisateur_id as user_id 
                               FROM notifications n 
                               LEFT JOIN utilisateurs u ON n.user = u.utilisateur_id 
                               WHERE n.id = ?");
        $stmt->execute([$id]);
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($data) {
            $stmt = $pdo->prepare("SELECT user FROM vue WHERE notification = ?");
            $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['notification_filters']);
            header('Location: ' . $_SERVER['PHP_SELF']);
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['notification_filters'] = [
        'search' => $search,
        'user' => $filter_user,
        'date_debut' => $filter_date_debut,
        'date_fin' => $filter_date_fin
    ];
} elseif (isset($_SESSION['notification_filters']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $search = $_SESSION['notification_filters']['search'] ?? '';
    $filter_user = $_SESSION['notification_filters']['user'] ?? '';
    $filter_date_debut = $_SESSION['notification_filters']['date_debut'] ?? '';
    $filter_date_fin = $_SESSION['notification_filters']['date_fin'] ?? '';
    $page = $_SESSION['notification_filters']['page'] ?? 1;
    $offset = ($page - 1) * $limit;
}
...
// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM notifications n LEFT JOIN utilisateurs u ON n.user = u.utilisateur_id';
$count_params = [];
$where_clauses = [];

if (!empty($search)) {
    $where_clauses[] = '(n.objet LIKE ? OR n.titre LIKE ? OR n.text LIKE ? OR u.nom_prenom LIKE ?)';
    $search_term = "%$search%";
    $count_params = array_merge($count_params, [$search_term, $search_term, $search_term, $search_term]);
}
if (!empty($filter_user)) {
    $where_clauses[] = 'n.user = ?';
    $count_params[] = $filter_user;
}
if (!empty($filter_date_debut)) {
    $where_clauses[] = 'DATE(n.date) >= ?';
...
$sql = 'SELECT n.*, u.nom_prenom, u.utilisateur_id as user_id 
        FROM notifications n 
        LEFT JOIN utilisateurs u ON n.user = u.utilisateur_id';
$params = [];
$where_clauses = [];
...
$sql .= ' ORDER BY n.date DESC, n.id DESC LIMIT ' . (int)$limit . ' OFFSET ' . (int)$offset;
...
$stmt = $pdo->query("SELECT COUNT(DISTINCT user) FROM notifications");
$total_utilisateurs = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM notifications WHERE DATE(date) = CURDATE()");
$aujourdhui = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM notifications WHERE DATE(date) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)");
$semaine = $stmt->fetchColumn();

// Dernière notification
$stmt = $pdo->query("SELECT n.*, u.nom_prenom, u.utilisateur_id as user_id 
                     FROM notifications n 
                     LEFT JOIN utilisateurs u ON n.user = u.utilisateur_id 
...
$utilisateurs_list = $pdo->query("SELECT utilisateur_id, nom_prenom, email FROM utilisateurs WHERE etat = 'actif' ORDER BY nom_prenom")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Gestion des Notifications - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .last-notification .last-date {
            font-size: 12px;
            color: var(--medical-text-muted);
        }

        .last-notification .last-title {
            font-size: 16px;
            font-weight: 700;
            color: var(--medical-text);
            margin: 4px 0;
        ...
        .last-notification .last-text {
            font-size: 14px;
            color: var(--medical-text-secondary);
            margin-bottom: 4px;
        }

        .last-notification .last-user {
            font-size: 12px;
            color: var(--medical-text-muted);
        }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">notifications</span></h1>
            <div class="subtitle">
                <i class="bi bi-bell"></i>
                Consulter, filtrer et gérer les notifications
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouvelle notification</span>
            </button>
        </div>
    </header>
    ...
    <div class="stats-grid">
        <div class="stat-card fade-in fade-in-d1">
            <div class="stat-icon info"><i class="bi bi-envelope"></i></div>
        ...
        <div class="stat-card fade-in fade-in-d2">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= number_format($aujourdhui) ?></div>
                <div class="stat-label">Aujourd'hui</div>
                <div class="stat-sub">Notifications envoyées</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon blue"><i class="bi bi-calendar-week"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($semaine) ?></div>
        ...
        <div class="stat-card fade-in fade-in-d4">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total_utilisateurs) ?></div>
                <div class="stat-label">Utilisateurs concernés</div>
                <div class="stat-sub">Destinataires uniques</div>
            </div>
        </div>
    </div>
    ...
    <!-- ============================================================ -->
    <?php if ($derniere): ?>
    <div class="last-notification fade-in">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <div class="last-date">
                    <i class="bi bi-clock me-1"></i>
                    Dernière notification - <?= date('d/m/Y H:i', strtotime($derniere['date'])) ?>
                </div>
                <div class="last-title"><?= htmlspecialchars($derniere['titre']) ?></div>
                <div class="last-text"><?= htmlspecialchars(substr($derniere['text'] ?? '', 0, 150)) ?>...</div>
                <div class="last-user">
                    <i class="bi bi-person-circle me-1"></i>
                    Destinataire : <?= htmlspecialchars($derniere['nom_prenom'] ?? 'Inconnu') ?>
                </div>
            </div>
            <button class="btn-medical-secondary btn-sm-medical" 
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Objet, titre, message...">
            </div>
            <div class="col-md-3">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des notifications
                <span class="badge bg-secondary ms-2"><?= number_format($total_notifications) ?></span>
            </h5>
            <?php if (!empty($active_filters)): ?>
                ...
                <table class="table table-dashboard">
                    ...
                    <tbody>
                        <?php if (empty($notifications)): ?>
                            <tr>
                                <td colspan="8" class="text-center py-5" style="color:var(--medical-text-muted);">
                                    ...
                                    <?php if ($total == 0): ?>
                                        <strong>Aucune notification enregistrée.</strong>
                                        <br>
                                        <span style="font-size:13px;">Cliquez sur "Nouvelle notification" pour en créer une.</span>
                                    <?php else: ?>
                                        <strong>Aucune notification trouvée avec ces filtres.</strong>
                                        <br>
                        ...
                        <?php else: foreach ($notifications as $n): ?>
                            <tr>
                                <td>
                                    <span class="fw-bold" style="color:var(--medical-text-secondary);">#<?= $n['id'] ?></span>
                                </td>
                                <td>
                                    <span class="badge bg-info"><?= htmlspecialchars($n['objet'] ?? 'N/A') ?></span>
                                </td>
                                <td><strong><?= htmlspecialchars($n['titre'] ?? 'N/A') ?></strong></td>
                                <td>
                                    <div style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($n['text'] ?? '') ?>">
                                        <?= htmlspecialchars($n['text'] ?? '') ?>
                                    </div>
                                </td>
                                <td><small><?= date('d/m/Y H:i', strtotime($n['date'] ?? 'now')) ?></small></td>
                                <td><?= htmlspecialchars($n['nom_prenom'] ?? 'Inconnu') ?></td>
                                <td>
                                    <?php if (!empty($n['fichier'])): ?>
                                        <a href="../uploads/notifications/<?= htmlspecialchars($n['fichier']) ?>" target="_blank" class="btn-medical-outline" style="padding:2px 8px;font-size:0.7rem;min-height:24px;">
                ...
                <div class="d-flex justify-content-center p-3">
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>">
                        ...
                        if ($start > 1): ?>
                            <li class="page-item">
                                <a class="page-link" href="?page=1&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>">1</a>
                            </li>
                            <?php if ($start > 2): ?>
                        ...
                        <?php for ($i = $start; $i <= $end; $i++): ?>
                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                        <?php if ($end < $total_pages): ?>
                            ...
                            <?php endif; ?>
                            <li class="page-item">
                                <a class="page-link" href="?page=<?= $total_pages ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>"><?= $total_pages ?></a>
                            </li>
                        <?php endif; ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Nouvelle notification</h5>
            ...
            <form method="POST" enctype="multipart/form-data" id="addForm">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="sai_objet" required maxlength="100" placeholder="Ex: Rapport mensuel">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Titre <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre" required maxlength="200" placeholder="Ex: Rapport du mois de Janvier">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">Message <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="sai_text" rows="5" required placeholder="Contenu de la notification..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            ...
                            <select name="sai_users[]" id="addUsers" class="form-select" multiple="multiple" required style="width:100%;">
                                <?php foreach ($utilisateurs_list as $u): ?>
                            ...
                            <div class="mt-2">
                                <small class="text-muted"><i class="bi bi-info-circle"></i> Sélectionnez plusieurs destinataires</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            ...
                            <input type="file" class="form-control" name="sai_fichier" id="addFichier" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="btn_ajouter" class="btn-medical-secondary" id="addSubmitBtn">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la notification</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette notification ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <span style="font-weight:600;color:var(--medical-text);">Titre :</span>
                    ...
                    <div class="alert-medical alert-info mt-3" style="font-size:12px;text-align:left;">
                        ...
                        <strong>Attention :</strong> Cette action est irréversible. Tous les suivis de lecture seront également supprimés.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
...
$(document).ready(function() {
    ...
    $('#addUsers').on('change', function() {
        ...
        if (count > 0) {
            btn.html('<i class="bi bi-send me-1"></i>Envoyer à ' + count + ' destinataire' + (count > 1 ? 's' : ''));
        } else {
            btn.html('<i class="bi bi-send me-1"></i>Envoyer');
        }
    });
    ...
    $('#addForm').on('submit', function(e) {
        var selected = $('#addUsers').val();
        if (!selected || selected.length === 0) {
            e.preventDefault();
            alert('⚠️ Veuillez sélectionner au moins un destinataire.');
            $('#addUsers').next('.select2-container').find('.select2-selection').css('border-color', 'var(--danger)');
            setTimeout(function() {
                $('#addUsers').next('.select2-container').find('.select2-selection').css('border-color', '');
            }, 3000);
            return false;
        }
        return true;
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de la notification...</p>
            </div>
        `);
        submitBtn.hide();
        ...
        $.post(window.location.href, { 
            ...
            if (data.success) {
                const n = data.data;
                const dest = n.destinataires || [];
                
                let optionsHtml = '';
                <?php foreach ($utilisateurs_list as $u): ?>
                    optionsHtml += `<option value="<?= htmlspecialchars($u['utilisateur_id']) ?>" ${dest.includes('<?= htmlspecialchars($u['utilisateur_id']) ?>') ? 'selected' : ''}>
                        <?= htmlspecialchars($u['nom_prenom']) ?> (<?= htmlspecialchars($u['email'] ?? '') ?>)
                    </option>`;
                <?php endforeach; ?>
                
                body.html(`
                    <input type="hidden" name="sai_id" value="${n.id}">
                    
                    <div class="section-title"><i class="bi bi-envelope-paper"></i> Informations</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Objet <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_objet" value="${n.objet || ''}" required maxlength="100">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Titre <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre" value="${n.titre || ''}" required maxlength="200">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">Message <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="sai_text" rows="5" required>${n.text || ''}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            <label class="form-label">Sélectionner les destinataires <span class="text-danger">*</span></label>
                            <select name="sai_users[]" id="editUsers" class="form-select" multiple="multiple" required style="width:100%;">
                                ${optionsHtml}
                            </select>
                            <div class="mt-2">
                                <small class="text-muted"><i class="bi bi-info-circle"></i> Sélectionnez plusieurs destinataires</small>
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            <label class="form-label">Fichier (optionnel)</label>
                            <input type="file" class="form-control" name="sai_fichier" id="editFichier" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif">
                            ${n.fichier ? `
                            <div class="fichier-info mt-2" style="background:var(--medical-blue-light);border-left:4px solid var(--medical-blue);padding:12px 16px;border-radius:var(--medical-radius-sm);">
                                <strong style="display:block;color:var(--medical-blue);font-size:0.8rem;text-transform:uppercase;letter-spacing:0.3px;margin-bottom:4px;"><i class="bi bi-file-earmark me-1"></i> Fichier actuel</strong>
                                <div class="d-flex flex-wrap gap-2 mt-1">
                                    <span style="font-family:monospace;font-size:13px;color:var(--medical-text);">${n.fichier}</span>
                                    <a href="../uploads/notifications/${n.fichier}" target="_blank" class="btn-medical-outline btn-sm-medical" style="padding:4px 12px;font-size:0.8rem;min-height:28px;">
                ...
                setTimeout(function() {
                    var count = $('#editUsers').val() ? $('#editUsers').val().length : 0;
                    var btn = $('#editSubmitBtn');
                    if (count > 0) {
                        btn.html('<i class="bi bi-check-circle me-1"></i>Enregistrer pour ' + count + ' destinataire' + (count > 1 ? 's' : ''));
                    } else {
                        btn.html('<i class="bi bi-check-circle me-1"></i>Enregistrer');
                    }
                }, 200);
                ...
                body.html(`
                    <div class="alert-medical alert-danger">
                        ...
                        ${data.message || 'Erreur lors du chargement de la notification.'}
                    </div>
                `);
            }
        }, 'json');
    ...
    // ============================================================
    $(document).on('change', '#editUsers', function() {
        var count = $(this).val() ? $(this).val().length : 0;
        var btn = $('#editSubmitBtn');
        if (count > 0) {
            btn.html('<i class="bi bi-check-circle me-1"></i>Enregistrer pour ' + count + ' destinataire' + (count > 1 ? 's' : ''));
