<?php
// ================================================================
// GESTION DES PRESTATIONS - EPENCIA SGI
// ================================================================
...
function getTypeBadge($type) {
    $typeMap = [
        'soins' => ['class' => 'primary', 'icon' => 'bi-heart-pulse'],
        'examen' => ['class' => 'warning', 'icon' => 'bi-microscope'],
        'médicament' => ['class' => 'success', 'icon' => 'bi-capsule']
    ];
    $info = $typeMap[$type] ?? ['class' => 'secondary', 'icon' => 'bi-box'];
    return '<span class="badge bg-' . $info['class'] . '"><i class="bi ' . $info['icon'] . ' me-1"></i>' . htmlspecialchars($type) . '</span>';
}

function getProduits($pdo) {
    try {
        $stmt = $pdo->query('SELECT produit_id, nom, type FROM produits WHERE statut = "actif" ORDER BY nom');
...
function getOrganismes($pdo) {
    try {
        $stmt = $pdo->query('SELECT organisme_id, nom, type FROM organismes WHERE statut = "actif" ORDER BY nom');
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        return [];
    }
...
$filter_type_produit = trim($_POST['type_produit'] ?? $_GET['type_produit'] ?? '');
$filter_organisme = trim($_POST['organisme_id'] ?? $_GET['organisme_id'] ?? '');
$filter_produit = trim($_POST['produit_id'] ?? $_GET['produit_id'] ?? '');
$filter_date_debut = trim($_POST['date_debut'] ?? $_GET['date_debut'] ?? '');
$filter_date_fin = trim($_POST['date_fin'] ?? $_GET['date_fin'] ?? '');
$filter_prix_min = trim($_POST['prix_min'] ?? $_GET['prix_min'] ?? '');
$filter_prix_max = trim($_POST['prix_max'] ?? $_GET['prix_max'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['prestations_filters'] = [
        'search' => $search,
        'statut' => $filter_statut,
        'type_produit' => $filter_type_produit,
        'organisme_id' => $filter_organisme,
        'produit_id' => $filter_produit,
...
} elseif (isset($_SESSION['prestations_filters']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $search = $_SESSION['prestations_filters']['search'] ?? '';
    $filter_statut = $_SESSION['prestations_filters']['statut'] ?? '';
    $filter_type_produit = $_SESSION['prestations_filters']['type_produit'] ?? '';
    $filter_organisme = $_SESSION['prestations_filters']['organisme_id'] ?? '';
    $filter_produit = $_SESSION['prestations_filters']['produit_id'] ?? '';
    $filter_date_debut = $_SESSION['prestations_filters']['date_debut'] ?? '';
    $filter_date_fin = $_SESSION['prestations_filters']['date_fin'] ?? '';
    $filter_prix_min = $_SESSION['prestations_filters']['prix_min'] ?? '';
    $filter_prix_max = $_SESSION['prestations_filters']['prix_max'] ?? '';
    $page = $_SESSION['prestations_filters']['page'] ?? 1;
    $offset = ($page - 1) * $limit;
}

// Construction de la requête
$sql = 'SELECT p.*, pr.nom as produit_nom, pr.type as produit_type, o.nom as organisme_nom, o.type as organisme_type 
        FROM prestations p 
        LEFT JOIN produits pr ON p.produit_id = pr.produit_id 
        LEFT JOIN organismes o ON p.organisme_id = o.organisme_id';
$where = [];
$params = [];

if (!empty($search)) {
    $where[] = '(p.prestation_id LIKE ? OR pr.nom LIKE ? OR o.nom LIKE ?)';
    $search_term = "%$search%";
    $params[] = $search_term;
    $params[] = $search_term;
    $params[] = $search_term;
...
if (!empty($filter_type_produit)) {
    $where[] = 'pr.type = ?';
    $params[] = $filter_type_produit;
}
if (!empty($filter_organisme)) {
    $where[] = 'p.organisme_id = ?';
    $params[] = $filter_organisme;
}
if (!empty($filter_produit)) {
    $where[] = 'p.produit_id = ?';
    $params[] = $filter_produit;
}
...
if (!empty($filter_prix_min)) {
    $where[] = 'p.prix >= ?';
    $params[] = $filter_prix_min;
}
if (!empty($filter_prix_max)) {
    $where[] = 'p.prix <= ?';
    $params[] = $filter_prix_max;
}
...
// Compter le total
$count_sql = str_replace('SELECT p.*, pr.nom as produit_nom, pr.type as produit_type, o.nom as organisme_nom, o.type as organisme_type', 'SELECT COUNT(*)', $sql);
$count_stmt = $pdo->prepare($count_sql);
$count_stmt->execute($params);
$total_prestations = $count_stmt->fetchColumn();
$total_pages = max(1, ceil($total_prestations / $limit));

// Récupérer les prestations avec pagination
$sql .= ' ORDER BY p.date DESC LIMIT ' . (int)$limit . ' OFFSET ' . (int)$offset;
...
// 4. STATISTIQUES
// ================================================================
$stmt = $pdo->query('SELECT COUNT(*) FROM prestations');
$total_general = $stmt->fetchColumn();

$stmt = $pdo->query('SELECT COUNT(*) FROM prestations WHERE statut = "actif"');
$total_actif = $stmt->fetchColumn();

$stmt = $pdo->query('SELECT COUNT(*) FROM prestations WHERE statut = "inactif"');
$total_inactif = $stmt->fetchColumn();

$stmt = $pdo->query('SELECT SUM(prix) as total FROM prestations');
$total_montant = $stmt->fetchColumn() ?? 0;

$liste_produits = getProduits($pdo);
$liste_organismes = getOrganismes($pdo);
$types_produits = ['soins', 'examen', 'médicament'];
$statuts_prestation = ['actif', 'inactif'];
...
if (!empty($filter_type_produit)) $active_filters[] = ['label' => 'Type Produit', 'value' => ucfirst($filter_type_produit)];
if (!empty($filter_produit)) {
    $produit_label = '';
    foreach ($liste_produits as $p) { if ($p['produit_id'] == $filter_produit) { $produit_label = $p['nom']; break; } }
    $active_filters[] = ['label' => 'Produit', 'value' => $produit_label ?: $filter_produit];
}
if (!empty($filter_organisme)) {
    $organisme_label = '';
    foreach ($liste_organismes as $o) { if ($o['organisme_id'] == $filter_organisme) { $organisme_label = $o['nom']; break; } }
    $active_filters[] = ['label' => 'Organisme', 'value' => $organisme_label ?: $filter_organisme];
}
...
if (!empty($filter_prix_min) || !empty($filter_prix_max)) {
    $prix_range = '';
    if (!empty($filter_prix_min)) $prix_range .= number_format($filter_prix_min, 0, ',', ' ') . ' FCFA';
    if (!empty($filter_prix_max)) $prix_range .= ($prix_range ? ' à ' : '') . number_format($filter_prix_max, 0, ',', ' ') . ' FCFA';
    if ($prix_range) $active_filters[] = ['label' => 'Prix', 'value' => $prix_range];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'add') {
            $prestation_id = trim($_POST['prestation_id']);
            ...
            if (empty($prestation_id) || empty($produit_id) || empty($organisme_id) || $prix <= 0 || empty($date)) {
                throw new Exception('Tous les champs obligatoires doivent être remplis correctement.');
            }

            $stmt = $pdo->prepare('SELECT COUNT(*) FROM prestations WHERE prestation_id = ?');
            $stmt->execute([$prestation_id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cet ID de prestation existe déjà.');
            }

            $stmt = $pdo->prepare('INSERT INTO prestations (prestation_id, produit_id, organisme_id, prix, date, statut) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([$prestation_id, $produit_id, $organisme_id, $prix, $date, $statut]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'edit') {
            ...
            $stmt = $pdo->prepare('UPDATE prestations SET prestation_id=?, produit_id=?, organisme_id=?, prix=?, date=?, statut=? WHERE prestation_id=?');
            $stmt->execute([$prestation_id, $produit_id, $organisme_id, $prix, $date, $statut, $id]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'delete') {
            $id = $_POST['id'];
            $stmt = $pdo->prepare('DELETE FROM prestations WHERE prestation_id = ?');
            $stmt->execute([$id]);
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['prestations_filters']);
            header('Location: ' . $_SERVER['PHP_SELF']);
            exit;
        }
        ...
        if (isset($_POST['export_excel'])) {
            ...
            header('Content-Disposition: attachment; filename="prestations_' . date('Ymd_His') . '.xls"');
            
            $sql_export = 'SELECT p.prestation_id, p.prix, p.date, p.statut, pr.nom as produit_nom, pr.type as produit_type, o.nom as organisme_nom, o.type as organisme_type 
                           FROM prestations p 
                           LEFT JOIN produits pr ON p.produit_id = pr.produit_id 
                           LEFT JOIN organismes o ON p.organisme_id = o.organisme_id
                           ORDER BY p.date DESC';
            $export_stmt = $pdo->query($sql_export);
            ...
            foreach ($export_data as $row) {
                echo '<tr>
                        <td>' . htmlspecialchars($row['prestation_id']) . '</td>
                        <td>' . htmlspecialchars($row['produit_nom']??'-') . '</td>
                        <td>' . htmlspecialchars($row['produit_type']??'-') . '</td>
                        <td>' . htmlspecialchars($row['organisme_nom']??'-') . '</td>
                        <td>' . htmlspecialchars($row['organisme_type']??'-') . '</td>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_prestation') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare('
            SELECT p.*, pr.nom as produit_nom, pr.type as produit_type, o.nom as organisme_nom, o.type as organisme_type 
            FROM prestations p 
            LEFT JOIN produits pr ON p.produit_id = pr.produit_id 
            LEFT JOIN organismes o ON p.organisme_id = o.organisme_id 
            WHERE p.prestation_id = ?
        ');
        $stmt->execute([$id]);
        $prestation = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($prestation) {
            echo json_encode(['success' => true, 'data' => $prestation]);
        } else {
...
<head>
    ...
    <title>Gestion des prestations - Epencia SGI</title>
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">prestations</span></h1>
            <div class="subtitle">
                <i class="bi bi-bag-check"></i>
                Créer, modifier et suivre les prestations des organismes
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouvelle prestation</span>
            </button>
    ...
    <div class="stats-grid">
        <div class="stat-card fade-in fade-in-d1">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total_general) ?></div>
        ...
        <div class="stat-card fade-in fade-in-d3">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Prestations inactives</div>
            </div>
        </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID prestation, produit, organisme...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Statut</label>
                <select name="statut" class="form-select">
                    <option value="">Tous les statuts</option>
                    <?php foreach ($statuts_prestation as $s): ?>
            ...
            <div class="col-md-3">
                ...
                <select name="type_produit" class="form-select">
                    ...
                    <?php foreach ($types_produits as $t): ?>
                        <option value="<?= $t ?>" <?= $filter_type_produit == $t ? 'selected' : '' ?>><?= ucfirst($t) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="produit_id" class="form-select">
                    ...
                    <?php foreach ($liste_produits as $p): ?>
                        <option value="<?= htmlspecialchars($p['produit_id']) ?>" <?= $filter_produit == $p['produit_id'] ? 'selected' : '' ?>>
            ...
            <div class="col-md-4">
                ...
                <div class="input-group">
                    <input type="date" name="date_debut" class="form-control" value="<?= htmlspecialchars($filter_date_debut) ?>" placeholder="Début">
                    <span class="input-group-text" style="background:var(--medical-gray-light);border-color:var(--medical-border);color:var(--medical-text-muted);">à</span>
                    <input type="date" name="date_fin" class="form-control" value="<?= htmlspecialchars($filter_date_fin) ?>" placeholder="Fin">
                </div>
            </div>
            <div class="col-md-2">
                <label class="form-label">Prix min (FCFA)</label>
                <input type="number" step="1" name="prix_min" class="form-control" value="<?= htmlspecialchars($filter_prix_min) ?>" placeholder="0">
            </div>
            <div class="col-md-2">
                <label class="form-label">Prix max (FCFA)</label>
                <input type="number" step="1" name="prix_max" class="form-control" value="<?= htmlspecialchars($filter_prix_max) ?>" placeholder="0">
            </div>
            <div class="col-12">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des prestations
                <span class="badge bg-secondary ms-2"><?= number_format($total_prestations) ?></span>
            </h5>
            <?php if (!empty($active_filters)): ?>
                <form method="POST" class="d-inline">
                ...
                <table class="table table-dashboard">
                    ...
                    <tbody>
                        <?php if (empty($prestations)): ?>
                            <tr>
                                <td colspan="7" class="text-center py-5" style="color:var(--medical-text-muted);">
                                    <i class="bi bi-inbox" style="font-size:3rem;display:block;margin-bottom:12px;opacity:0.3;"></i>
                                    <?php if ($total_general == 0): ?>
                                        <strong>Aucune prestation enregistrée.</strong>
                                        <br>
                                        <span style="font-size:13px;">Cliquez sur "Nouvelle prestation" pour en créer une.</span>
                                    <?php else: ?>
                                        <strong>Aucune prestation trouvée avec ces filtres.</strong>
                                        <br>
                                        <span style="font-size:13px;">Essayez de modifier vos filtres de recherche.</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php else: foreach ($prestations as $p): ?>
                            <tr>
                                <td>
                                    <code style="font-size:clamp(11px,0.75vw,13px);color:var(--medical-blue);font-weight:600;background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;">
                                        <?= htmlspecialchars($p['prestation_id']) ?>
                                    </code>
                                </td>
                                <td>
                                    <strong><?= htmlspecialchars($p['produit_nom'] ?? '-') ?></strong>
                                    <br>
                                    <?= getTypeBadge($p['produit_type'] ?? '') ?>
                                </td>
                                <td>
                                    <strong><?= htmlspecialchars($p['organisme_nom'] ?? '-') ?></strong>
                                    <br>
                                    <small class="text-muted"><?= ucfirst($p['organisme_type'] ?? '-') ?></small>
                                </td>
                                ...
                                <td>
                                    <div class="actions-group">
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= htmlspecialchars($p['prestation_id']) ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= htmlspecialchars($p['prestation_id']) ?>" 
                                            data-produit="<?= htmlspecialchars($p['produit_nom'] ?? '-') ?>" 
                                            title="Supprimer">
                ...
                <div class="d-flex justify-content-center p-3">
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filter_statut) ?>&type_produit=<?= urlencode($filter_type_produit) ?>&organisme_id=<?= urlencode($filter_organisme) ?>&produit_id=<?= urlencode($filter_produit) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>&prix_min=<?= urlencode($filter_prix_min) ?>&prix_max=<?= urlencode($filter_prix_max) ?>">
                                <i class="bi bi-chevron-left"></i>
                            </a>
                        </li>
                        <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filter_statut) ?>&type_produit=<?= urlencode($filter_type_produit) ?>&organisme_id=<?= urlencode($filter_organisme) ?>&produit_id=<?= urlencode($filter_produit) ?>&date_debut=<?= urlencode($filter_date_debut) ?>&date_fin=<?= urlencode($filter_date_fin) ?>&prix_min=<?= urlencode($filter_prix_min) ?>&prix_max=<?= urlencode($filter_prix_max) ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Nouvelle prestation</h5>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Prestation <span class="text-danger">*</span></label>
                            <input type="text" name="prestation_id" class="form-control" required placeholder="Ex: PREST-001">
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="section-title"><i class="bi bi-box-seam"></i> Produit et Organisme</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select name="produit_id" class="form-select" required>
                                ...
                                <?php foreach ($liste_produits as $p): ?>
                                    <option value="<?= htmlspecialchars($p['produit_id']) ?>"><?= htmlspecialchars($p['nom']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Organisme <span class="text-danger">*</span></label>
                            <select name="organisme_id" class="form-select" required>
                                <option value="">Sélectionnez un organisme</option>
                                <?php foreach ($liste_organismes as $o): ?>
                                    <option value="<?= htmlspecialchars($o['organisme_id']) ?>"><?= htmlspecialchars($o['nom']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>

                    <div class="section-title"><i class="bi bi-cash-coin"></i> Informations financières</div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Prix (FCFA) <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="prix" class="form-control" required min="0" placeholder="0">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Statut</label>
                            <select name="statut" class="form-select">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la prestation</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette prestation ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_prestation_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Produit :</span>
                            <span style="font-weight:500;" id="delete_prestation_produit"></span>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de la prestation...</p>
            </div>
        `);
        submitBtn.hide();
        
        $.post(window.location.href, { 
            ajax_action: 'load_prestation', 
            id: id 
        }, function(data) {
            if (data.success) {
                const p = data.data;
                body.html(`
                    ...
                    <input type="hidden" name="id" value="${p.prestation_id}">
                    
                    <div class="section-title"><i class="bi bi-fingerprint"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Prestation</label>
                            <input type="text" name="prestation_id" class="form-control bg-light" value="${p.prestation_id}" readonly style="font-family:monospace;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Date <span class="text-danger">*</span></label>
                            <input type="date" name="date" class="form-control" value="${p.date}" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select name="produit_id" class="form-select" required>
                                ...
                                <?php foreach ($liste_produits as $prod): ?>
                                    <option value="<?= htmlspecialchars($prod['produit_id']) ?>" ${p.produit_id === '<?= $prod['produit_id'] ?>' ? 'selected' : ''}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="organisme_id" class="form-select" required>
                                <option value="">Sélectionnez un organisme</option>
                                <?php foreach ($liste_organismes as $org): ?>
                                    <option value="<?= htmlspecialchars($org['organisme_id']) ?>" ${p.organisme_id === '<?= $org['organisme_id'] ?>' ? 'selected' : ''}>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Prix (FCFA) <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="prix" class="form-control" value="${p.prix}" required min="0">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Statut</label>
                            <select name="statut" class="form-select">
                                <?php foreach ($statuts_prestation as $s): ?>
                                    <option value="<?= $s ?>" ${p.statut === '<?= $s ?>' ? 'selected' : ''}>${esc('<?= ucfirst($s) ?>')}</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                ...
                body.html(`
                    <div class="alert-medical alert-danger">
                        ...
                        Erreur lors du chargement de la prestation.
                    </div>
                `);
            }
        }, 'json');
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        const button = $(e.relatedTarget);
        const id = button.data('id');
        const produit = button.data('produit');
        
        $('#delete_id').val(id);
        $('#delete_prestation_id').text(id);
        $('#delete_prestation_produit').text(produit);
    });
});
