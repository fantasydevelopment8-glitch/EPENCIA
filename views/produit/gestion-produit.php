<?php
// ================================================================
// GESTION DES PRODUITS - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['produits_filters'] = [
        'search' => $search,
        'type' => $filter_type,
        'statut' => $filter_statut,
        'page' => $page
    ];
} elseif (isset($_SESSION['produits_filters']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $search = $_SESSION['produits_filters']['search'] ?? '';
    $filter_type = $_SESSION['produits_filters']['type'] ?? '';
    $filter_statut = $_SESSION['produits_filters']['statut'] ?? '';
    $page = $_SESSION['produits_filters']['page'] ?? 1;
    $offset = ($page - 1) * $limit;
}
...
if (!empty($search)) {
    $where[] = '(produit_id LIKE ? OR nom LIKE ? OR description LIKE ?)';
    $search_term = "%$search%";
    $params[] = $search_term;
    $params[] = $search_term;
    $params[] = $search_term;
...
$total_pages = max(1, ceil($total_produits / $limit));

// Récupérer les produits avec pagination - CORRECTION ICI
...
$stmt = $pdo->query('SELECT COUNT(*) FROM produits WHERE statut = "inactif"');
...
foreach ($types_stats_raw as $s) {
    $types_stats[$s['type']] = $s['count'];
}

$types_produit = ['soins', 'examen', 'médicament'];
$statuts_produit = ['actif', 'inactif'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'add') {
            ...
            if (empty($produit_id) || empty($nom) || empty($type)) {
                throw new Exception('Tous les champs obligatoires doivent être remplis.');
            }

            $stmt = $pdo->prepare('SELECT COUNT(*) FROM produits WHERE produit_id = ?');
            $stmt->execute([$produit_id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cet ID de produit existe déjà.');
            }

            $stmt = $pdo->prepare('INSERT INTO produits (produit_id, nom, type, description, statut) VALUES (?, ?, ?, ?, ?)');
            $stmt->execute([$produit_id, $nom, $type, $description, $statut]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'edit') {
            ...
            $stmt = $pdo->prepare('UPDATE produits SET produit_id = ?, nom = ?, type = ?, description = ?, statut = ? WHERE produit_id = ?');
            $stmt->execute([$produit_id, $nom, $type, $description, $statut, $id]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'delete') {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM commandes WHERE produit_id = ?');
            $stmt->execute([$id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Impossible de supprimer un produit lié à des commandes.');
            }
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['produits_filters']);
            header('Location: ' . $_SERVER['PHP_SELF']);
            exit;
        }
        ...
        if (isset($_POST['export_excel'])) {
            ...
            $export_sql = 'SELECT * FROM produits ORDER BY nom ASC';
            ...
            foreach ($export_data as $row) {
                echo '<tr>';
                echo '<td>' . htmlspecialchars($row['produit_id']) . '</td>';
...
// ================================================================
// 6. HANDLERS AJAX (uniquement pour le chargement en modification)
// ================================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    header('Content-Type: application/json; charset=utf-8');
    
    if ($_POST['ajax_action'] === 'load_produit') {
        $id = $_POST['id'];
...
<head>
    ...
    <title>Gestion des produits - Epencia SGI</title>
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">produits</span></h1>
            <div class="subtitle">
                <i class="bi bi-box-seam"></i>
                Créer, modifier et gérer les produits, soins et médicaments
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouveau produit</span>
            </button>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d4">
            <div class="stat-icon info"><i class="bi bi-capsule"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($types_stats['médicament'] ?? 0) ?></div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID produit, nom, description...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Type</label>
                <select name="type" class="form-select">
                    <option value="">Tous les types</option>
                    <?php foreach ($types_produit as $t): ?>
            ...
            <div class="col-md-3">
                ...
                <select name="statut" class="form-select">
                    ...
                    <?php foreach ($statuts_produit as $s): ?>
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des produits
                <span class="badge bg-secondary ms-2"><?= number_format($total_produits) ?></span>
            </h5>
            <?php if (!empty($active_filters)): ?>
                <form method="POST" class="d-inline">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5" style="color:var(--medical-text-muted);">
                                    ...
                                    <?php if ($total_general == 0): ?>
                                        <strong>Aucun produit enregistré.</strong>
                                        <br>
                                        <span style="font-size:13px;">Cliquez sur "Nouveau produit" pour en créer un.</span>
                                    <?php else: ?>
                                        <strong>Aucun produit trouvé avec ces filtres.</strong>
                                        <br>
                ...
                <div class="d-flex justify-content-center p-3">
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&type=<?= urlencode($filter_type) ?>&statut=<?= urlencode($filter_statut) ?>">
                                <i class="bi bi-chevron-left"></i>
                            </a>
                        </li>
                        <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&type=<?= urlencode($filter_type) ?>&statut=<?= urlencode($filter_statut) ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Nouveau produit</h5>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Produit <span class="text-danger">*</span></label>
                            <input type="text" name="produit_id" class="form-control" required placeholder="Ex: PROD-001">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom <span class="text-danger">*</span></label>
                            <input type="text" name="nom" class="form-control" required placeholder="Nom du produit">
                        </div>
                    </div>

                    <div class="section-title"><i class="bi bi-info-circle"></i> Détails</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                    ...
                    <div class="section-title"><i class="bi bi-text-paragraph"></i> Description</div>
                    <div class="row g-3">
                        <div class="col-md-12">
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer ce produit ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_produit_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Nom :</span>
                            <span style="font-weight:500;" id="delete_produit_nom"></span>
                        </div>
                    </div>
                    
                    <div class="alert-medical alert-info mt-3" style="font-size:12px;text-align:left;">
                        <i class="bi bi-exclamation-triangle me-1"></i>
                        <strong>Attention :</strong> Cette action est irréversible. Si le produit est lié à des commandes, la suppression sera bloquée.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
...
$(document).ready(function() {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement du produit...</p>
            </div>
        `);
        submitBtn.hide();
        
        $.post(window.location.href, { 
            ajax_action: 'load_produit', 
            id: id 
        }, function(data) {
            if (data.success) {
                const p = data.data;
                body.html(`
                    ...
                    <input type="hidden" name="id" value="${p.produit_id}">
                    
                    <div class="section-title"><i class="bi bi-fingerprint"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Produit</label>
                            <input type="text" name="produit_id" class="form-control bg-light" value="${p.produit_id}" readonly style="font-family:monospace;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom <span class="text-danger">*</span></label>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select" required>
                                <?php foreach ($types_produit as $t): ?>
                                    <option value="<?= $t ?>" ${p.type === '<?= $t ?>' ? 'selected' : ''}>${esc('<?= ucfirst($t) ?>')}</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        $('#delete_id').val(id);
        $('#delete_produit_id').text(id);
        $('#delete_produit_nom').text(nom);
    });
});
