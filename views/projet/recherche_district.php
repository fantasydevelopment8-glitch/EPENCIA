<?php
// ================================================================
// ASSOCIATIONS PROJET/DISTRICT - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_affectation') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("SELECT pd.*, p.titre_projet, d.titre_district 
                               FROM projet_district pd 
                               LEFT JOIN projet p ON pd.id_projet = p.id_projet 
                               LEFT JOIN district d ON pd.id_district = d.id_district 
                               WHERE pd.id = ?");
        $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['btn_ajouter'])) {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM projet_district WHERE id_projet = ? AND id_district = ?');
            $stmt->execute([$id_projet, $id_district]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cette association projet/district existe déjà.');
            }
            
            $req = $pdo->prepare("INSERT INTO projet_district (id_projet, id_district) VALUES (?, ?)");
            $req->execute([$id_projet, $id_district]);
            
            $message = "Association projet/district créée avec succès !";
        }
        ...
        if (isset($_POST['btn_modifier'])) {
            ...
            if (empty($id) || empty($id_projet) || empty($id_district)) {
                throw new Exception('Les champs obligatoires doivent être remplis.');
            }
            
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM projet_district WHERE id_projet = ? AND id_district = ? AND id != ?');
            $stmt->execute([$id_projet, $id_district, $id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cette association projet/district existe déjà.');
            }
            
            $req = $pdo->prepare("UPDATE projet_district SET id_projet = ?, id_district = ? WHERE id = ?");
            $req->execute([$id_projet, $id_district, $id]);
            
            $message = 'Association projet/district modifiée avec succès.';
        }
        ...
        if (isset($_POST['btn_supprimer'])) {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM donnee WHERE id_projet = (SELECT id_projet FROM projet_district WHERE id = ?) AND id_district = (SELECT id_district FROM projet_district WHERE id = ?)');
            $stmt->execute([$id, $id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception("Cette association est utilisée dans des données.");
            }
            
            $req = $pdo->prepare("DELETE FROM projet_district WHERE id = ?");
            $req->execute([$id]);
            
            $message = 'Association projet/district supprimée avec succès.';
        }

        // RÉINITIALISER LES FILTRES
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['projet_district_filters']);
        }
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['projet_district_filters']['search'] ?? '');
$filter_projet = trim($_POST['projet'] ?? $_SESSION['projet_district_filters']['projet'] ?? '');
$filter_district = trim($_POST['district'] ?? $_SESSION['projet_district_filters']['district'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['projet_district_filters'] = [
        'search' => $search,
...
// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM projet_district pd 
              LEFT JOIN projet p ON pd.id_projet = p.id_projet 
              LEFT JOIN district d ON pd.id_district = d.id_district';
$count_params = [];
$where_clauses = [];
...
$sql = 'SELECT pd.id, pd.id_projet, pd.id_district, p.titre_projet, d.titre_district 
        FROM projet_district pd 
        LEFT JOIN projet p ON pd.id_projet = p.id_projet 
        LEFT JOIN district d ON pd.id_district = d.id_district';
$params = [];
...
$sql .= ' ORDER BY p.titre_projet ASC, d.titre_district ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM projet_district");
$total = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_projet) FROM projet_district");
$total_projets = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_district) FROM projet_district");
$total_districts = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT p.titre_projet, COUNT(*) as nb 
                     FROM projet_district pd 
                     LEFT JOIN projet p ON pd.id_projet = p.id_projet 
                     GROUP BY pd.id_projet 
                     ORDER BY nb DESC LIMIT 1");
$projet_plus_associe = $stmt->fetch(PDO::FETCH_ASSOC);

$stmt = $pdo->query("SELECT d.titre_district, COUNT(*) as nb 
                     FROM projet_district pd 
                     LEFT JOIN district d ON pd.id_district = d.id_district 
                     GROUP BY pd.id_district 
                     ORDER BY nb DESC LIMIT 1");
$district_plus_associe = $stmt->fetch(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Associations Projet/District - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 576px) {
            .btn-back-adaptive { width: 36px !important; height: 36px !important; padding: 0 !important; }
        }
        ...
        @media (max-width: 820px) {
            ...
            /* Réduction des boutons en tablette */
            .btn-medical-primary,
            .btn-medical-secondary,
            .btn-medical-danger,
            .btn-medical-outline {
        ...
        @media (max-width: 576px) {
            ...
            /* Réduction maximale des boutons */
            .btn-medical-primary,
            .btn-medical-secondary,
            .btn-medical-danger,
            .btn-medical-outline {
            ...
            .badge-projet, .badge-district { font-size: 10px; padding: 2px 8px; }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Associations <span class="highlight">Projet / District</span></h1>
            <div class="subtitle">
                <i class="bi bi-link"></i>
                Gérer les associations entre projets et districts
                <span class="dot"></span>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon teal"><i class="bi bi-geo-alt"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total_districts) ?></div>
                <div class="stat-label">Districts associés</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d4">
            <div class="stat-icon warning"><i class="bi bi-trophy"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= $projet_plus_associe ? $projet_plus_associe['nb'] : '0' ?></div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Projet, district...">
            </div>
            <div class="col-md-3">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des associations
                <span class="badge bg-secondary ms-2"><?= $total_affectations ?></span>
            </h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge-district">
                                        ...
                                        <?= htmlspecialchars($a['titre_district'] ?: 'N/A') ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="actions-group">
                                        ...
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            ...
                                            data-district="<?= htmlspecialchars($a['titre_district'] ?: 'N/A') ?>" 
                                            title="Supprimer">
                ...
                <div class="modal-body">
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <span class="badge-district"><i class="bi bi-geo-alt me-1"></i> <span id="delete_district"></span></span>
                        </div>
                    </div>
            ...
            if (data.success) {
                ...
                body.html(`
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">District <span class="text-danger">*</span></label>
                            <select name="sai_id_district" id="edit_sai_id_district" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un district --</option>
                                <?php foreach ($districts as $d): ?>
                                    <option value="<?= $d['id_district'] ?>" ${a.id_district === '<?= $d['id_district'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($d['titre_district']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
