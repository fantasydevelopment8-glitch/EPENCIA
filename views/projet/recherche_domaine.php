<?php
// ================================================================
// ASSOCIATIONS PROJET/DOMAINE - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_affectation') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("SELECT pd.*, p.titre_projet, d.titre_domaine 
                               FROM projet_domaine pd 
                               LEFT JOIN projet p ON pd.id_projet = p.id_projet 
                               LEFT JOIN domaine d ON pd.id_domaine = d.id_domaine 
                               WHERE pd.id = ?");
        $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['btn_ajouter'])) {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM projet_domaine WHERE id_projet = ? AND id_domaine = ?');
            $stmt->execute([$id_projet, $id_domaine]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cette association projet/domaine existe déjà.');
            }
            
            $req = $pdo->prepare("INSERT INTO projet_domaine (id_projet, id_domaine) VALUES (?, ?)");
            $req->execute([$id_projet, $id_domaine]);
            
            $message = "Association projet/domaine créée avec succès !";
        }
        ...
        if (isset($_POST['btn_modifier'])) {
            ...
            if (empty($id) || empty($id_projet) || empty($id_domaine)) {
                throw new Exception('Les champs obligatoires doivent être remplis.');
            }
            
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM projet_domaine WHERE id_projet = ? AND id_domaine = ? AND id != ?');
            $stmt->execute([$id_projet, $id_domaine, $id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cette association projet/domaine existe déjà.');
            }
            
            $req = $pdo->prepare("UPDATE projet_domaine SET id_projet = ?, id_domaine = ? WHERE id = ?");
            $req->execute([$id_projet, $id_domaine, $id]);
            
            $message = 'Association projet/domaine modifiée avec succès.';
        }
        ...
        if (isset($_POST['btn_supprimer'])) {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM donnee WHERE id_projet = (SELECT id_projet FROM projet_domaine WHERE id = ?) AND id_domaine = (SELECT id_domaine FROM projet_domaine WHERE id = ?)');
            $stmt->execute([$id, $id]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception("Cette association est utilisée dans des données.");
            }
            
            $req = $pdo->prepare("DELETE FROM projet_domaine WHERE id = ?");
            $req->execute([$id]);
            
            $message = 'Association projet/domaine supprimée avec succès.';
        }

        // RÉINITIALISER LES FILTRES
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['projet_domaine_filters']);
        }
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['projet_domaine_filters']['search'] ?? '');
$filter_projet = trim($_POST['projet'] ?? $_SESSION['projet_domaine_filters']['projet'] ?? '');
$filter_domaine = trim($_POST['domaine'] ?? $_SESSION['projet_domaine_filters']['domaine'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['projet_domaine_filters'] = [
        'search' => $search,
...
// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM projet_domaine pd 
              LEFT JOIN projet p ON pd.id_projet = p.id_projet 
              LEFT JOIN domaine d ON pd.id_domaine = d.id_domaine';
$count_params = [];
$where_clauses = [];
...
if (!empty($search)) {
    $where_clauses[] = '(p.titre_projet LIKE ? OR d.titre_domaine LIKE ?)';
    $search_term = "%$search%";
...
$sql = 'SELECT pd.id, pd.id_projet, pd.id_domaine, p.titre_projet, d.titre_domaine 
        FROM projet_domaine pd 
        LEFT JOIN projet p ON pd.id_projet = p.id_projet 
        LEFT JOIN domaine d ON pd.id_domaine = d.id_domaine';
$params = [];
...
$sql .= ' ORDER BY p.titre_projet ASC, d.titre_domaine ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM projet_domaine");
$total = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_projet) FROM projet_domaine");
$total_projets = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_domaine) FROM projet_domaine");
...
$stmt = $pdo->query("SELECT d.titre_domaine, COUNT(*) as nb 
                     FROM projet_domaine pd 
                     LEFT JOIN domaine d ON pd.id_domaine = d.id_domaine 
                     GROUP BY pd.id_domaine 
                     ORDER BY nb DESC LIMIT 1");
$domaine_plus_associe = $stmt->fetch(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Associations Projet/Domaine - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 576px) {
            ...
            .badge-projet, .badge-domaine { font-size: 10px; padding: 2px 8px; }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Associations <span class="highlight">Projet / Domaine</span></h1>
            <div class="subtitle">
                <i class="bi bi-link"></i>
                Gérer les associations entre projets et domaines
                <span class="dot"></span>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon teal"><i class="bi bi-tag"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total_domaines) ?></div>
                <div class="stat-label">Domaines associés</div>
            </div>
        </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Projet, domaine...">
            </div>
            <div class="col-md-3">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge-domaine">
                                        ...
                                        <?= htmlspecialchars($a['titre_domaine'] ?: 'N/A') ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="actions-group">
                                        ...
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            ...
                                            data-domaine="<?= htmlspecialchars($a['titre_domaine'] ?: 'N/A') ?>" 
                                            title="Supprimer">
                    ...
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <form method="POST" class="d-inline page-form">
                                ...
                                <input type="hidden" name="domaine" value="<?= htmlspecialchars($filter_domaine) ?>">
                ...
                <div class="modal-body">
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <span class="badge-domaine"><i class="bi bi-tag me-1"></i> <span id="delete_domaine"></span></span>
                        </div>
                    </div>
...
$(document).ready(function() {
            ...
            if (data.success) {
                ...
                body.html(`
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Domaine <span class="text-danger">*</span></label>
                            <select name="sai_id_domaine" id="edit_sai_id_domaine" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un domaine --</option>
                                <?php foreach ($domaines as $d): ?>
                                    <option value="<?= $d['id_domaine'] ?>" ${a.id_domaine === '<?= $d['id_domaine'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($d['titre_domaine']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        const domaine = button.data('domaine');
        
        $('#delete_id').val(id);
        $('#delete_projet').text(projet);
        $('#delete_domaine').text(domaine);
    });
