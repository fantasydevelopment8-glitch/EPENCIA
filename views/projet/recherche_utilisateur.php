<?php
// ================================================================
// ASSOCIATIONS UTILISATEUR/PROJET - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_affectation') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("SELECT up.*, u.nom_prenom, u.utilisateur_id, p.titre_projet 
                               FROM utilisateur_projet up 
                               LEFT JOIN utilisateurs u ON up.utilisateur = u.utilisateur_id 
                               LEFT JOIN projet p ON up.projet = p.id_projet 
                               WHERE up.id = ?");
        $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['utilisateur_projet_filters']);
        }
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['utilisateur_projet_filters']['search'] ?? '');
$filter_utilisateur = trim($_POST['utilisateur'] ?? $_SESSION['utilisateur_projet_filters']['utilisateur'] ?? '');
$filter_projet = trim($_POST['projet'] ?? $_SESSION['utilisateur_projet_filters']['projet'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['utilisateur_projet_filters'] = [
        'search' => $search,
        'utilisateur' => $filter_utilisateur,
...
// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM utilisateur_projet up 
              LEFT JOIN utilisateurs u ON up.utilisateur = u.utilisateur_id 
              LEFT JOIN projet p ON up.projet = p.id_projet';
$count_params = [];
$where_clauses = [];
...
if (!empty($search)) {
    $where_clauses[] = '(u.nom_prenom LIKE ? OR p.titre_projet LIKE ? OR up.zone LIKE ? OR up.localite LIKE ?)';
    $search_term = "%$search%";
    $count_params = array_merge($count_params, [$search_term, $search_term, $search_term, $search_term]);
}
if (!empty($filter_utilisateur)) {
    $where_clauses[] = 'up.utilisateur = ?';
    $count_params[] = $filter_utilisateur;
}
if (!empty($filter_projet)) {
    $where_clauses[] = 'up.projet = ?';
...
$sql = 'SELECT up.id, up.utilisateur, up.projet, up.zone, up.localite, 
               u.nom_prenom, u.utilisateur_id, p.titre_projet 
        FROM utilisateur_projet up 
        LEFT JOIN utilisateurs u ON up.utilisateur = u.utilisateur_id 
        LEFT JOIN projet p ON up.projet = p.id_projet';
$params = [];
...
$sql .= ' ORDER BY u.nom_prenom ASC, p.titre_projet ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM utilisateur_projet");
$total = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT utilisateur) FROM utilisateur_projet");
$total_utilisateurs = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT projet) FROM utilisateur_projet");
$total_projets = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT u.nom_prenom, COUNT(*) as nb 
                     FROM utilisateur_projet up 
                     LEFT JOIN utilisateurs u ON up.utilisateur = u.utilisateur_id 
                     GROUP BY up.utilisateur 
                     ORDER BY nb DESC LIMIT 1");
$utilisateur_plus_associe = $stmt->fetch(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Associations Utilisateur/Projet - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 576px) {
            ...
            .badge-utilisateur, .badge-projet { font-size: 10px; padding: 2px 8px; }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Associations <span class="highlight">Utilisateur / Projet</span></h1>
            <div class="subtitle">
                <i class="bi bi-people"></i>
                Gérer les associations entre utilisateurs et projets
                <span class="dot"></span>
    ...
    <div class="stats-grid">
        <div class="stat-card fade-in fade-in-d1">
            <div class="stat-icon info"><i class="bi bi-link"></i></div>
        ...
        <div class="stat-card fade-in fade-in-d2">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Utilisateurs associés</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon teal"><i class="bi bi-folder"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total_projets) ?></div>
        ...
        <div class="stat-card fade-in fade-in-d4">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= $utilisateur_plus_associe ? $utilisateur_plus_associe['nb'] : '0' ?></div>
                <div class="stat-label">Max par utilisateur</div>
                <div class="stat-sub"><?= $utilisateur_plus_associe ? htmlspecialchars($utilisateur_plus_associe['nom_prenom']) : '-' ?></div>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Utilisateur, projet, zone, localité...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Utilisateur</label>
                <select name="utilisateur" id="filterUtilisateur" class="form-select">
                    <option value="">Tous</option>
                    <?php foreach ($utilisateurs as $u): ?>
                        <option value="<?= $u['utilisateur_id'] ?>" <?= $filter_utilisateur == $u['utilisateur_id'] ? 'selected' : '' ?>>
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des associations
                <span class="badge bg-secondary ms-2"><?= number_format($total_affectations) ?></span>
            </h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge-utilisateur">
                                        ...
                                        <?= htmlspecialchars($a['nom_prenom'] ?? 'Inconnu') ?>
                                    </span>
                                </td>
                                <td>
                                    <span class="badge-projet">
                                        <i class="bi bi-folder me-1"></i>
                                        <?= htmlspecialchars($a['titre_projet'] ?? 'Inconnu') ?>
                                    </span>
                                </td>
                                <td><?= htmlspecialchars($a['zone'] ?? '—') ?></td>
                                <td><?= htmlspecialchars($a['localite'] ?? '—') ?></td>
                                <td>
                                    <div class="actions-group">
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= $a['id'] ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= $a['id'] ?>" 
                                            data-utilisateur="<?= htmlspecialchars($a['nom_prenom'] ?? 'Inconnu') ?>" 
                                            data-projet="<?= htmlspecialchars($a['titre_projet'] ?? 'Inconnu') ?>" 
                                            title="Supprimer">
                    ...
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <form method="POST" class="d-inline page-form">
                                ...
                                <input type="hidden" name="utilisateur" value="<?= htmlspecialchars($filter_utilisateur) ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Nouvelle association</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <div class="code-input-group">
                                <input type="text" class="form-control" id="add_sai_id" name="sai_id" 
                                       value="<?= htmlspecialchars($generated_id) ?>" required maxlength="20">
                                <button type="button" id="generateIdBtn" class="btn-generate-id">
                                    <i class="bi bi-arrow-repeat"></i>
                                </button>
                            </div>
                            <small class="text-muted">Format: UP + Année + 4 chiffres</small>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select name="sai_utilisateur" id="add_sai_utilisateur" class="form-select" required style="width:100%;">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="sai_projet" id="add_sai_projet" class="form-select" required style="width:100%;">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="sai_zone" required maxlength="100" placeholder="Ex: Zone Nord">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Localité <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_localite" required maxlength="100" placeholder="Ex: Abidjan">
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier l'association</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette association ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            <span class="badge-utilisateur"><i class="bi bi-person me-1"></i> <span id="delete_utilisateur"></span></span>
                            <span style="color:var(--medical-text-muted);">→</span>
                            <span class="badge-projet"><i class="bi bi-folder me-1"></i> <span id="delete_projet"></span></span>
                        </div>
                    </div>
                    
                    <p class="text-danger mb-0 mt-3" style="font-size:13px;font-weight:600;">
...
$(document).ready(function() {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de l'association...</p>
            </div>
        `);
                ...
                body.html(`
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" value="${a.id}" readonly style="font-family:monospace;font-weight:600;">
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Utilisateur <span class="text-danger">*</span></label>
                            <select name="sai_utilisateur" id="edit_sai_utilisateur" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un utilisateur --</option>
                                <?php foreach ($utilisateurs as $u): ?>
                                    <option value="<?= $u['utilisateur_id'] ?>" ${a.utilisateur === '<?= $u['utilisateur_id'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($u['nom_prenom']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Projet <span class="text-danger">*</span></label>
                            <select name="sai_projet" id="edit_sai_projet" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un projet --</option>
                                <?php foreach ($projets as $p): ?>
                                    <option value="<?= $p['id_projet'] ?>" ${a.projet === '<?= $p['id_projet'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($p['titre_projet']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Zone <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_zone" value="${a.zone || ''}" required maxlength="100" placeholder="Ex: Zone Nord">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Localité <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_localite" value="${a.localite || ''}" required maxlength="100" placeholder="Ex: Abidjan">
                        </div>
                    </div>
                `);
                
                $('#edit_sai_utilisateur').select2({
                    theme: 'default',
                    width: '100%',
                    placeholder: 'Sélectionnez un utilisateur',
                    allowClear: true,
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        const button = $(e.relatedTarget);
        const id = button.data('id');
        const utilisateur = button.data('utilisateur');
        const projet = button.data('projet');
        
        $('#delete_id').val(id);
        $('#delete_utilisateur').text(utilisateur);
