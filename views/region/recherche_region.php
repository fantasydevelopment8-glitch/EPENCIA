<?php
// ================================================================
// GESTION DES RÉGIONS - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_region') {
        $id = $_POST['id'];
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['region_filters']['search'] ?? '');
$filter_etat = trim($_POST['etat'] ?? $_SESSION['region_filters']['etat'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['region_filters'] = [
        'search' => $search,
        'etat' => $filter_etat
    ];
}
...
$total_regions = $stmt->fetchColumn();
$total_pages = max(1, ceil($total_regions / $limit));
...
$sql .= ' ORDER BY titre_region ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM region");
$total = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM region WHERE etat_region = 'ACTIF'");
$actifs = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM region WHERE etat_region = 'INACTIF'");
...
<head>
    ...
    <title>Gestion des Régions - Epencia SGI</title>
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">régions</span></h1>
            <div class="subtitle">
                <i class="bi bi-globe"></i>
                Consulter, ajouter, modifier et supprimer les régions
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouvelle région</span>
            </button>
        </div>
    </header>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d2">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Régions inactives</div>
            </div>
        </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID ou nom de la région...">
            </div>
            <div class="col-md-4">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des régions
                <span class="badge bg-secondary ms-2"><?= number_format($total_regions) ?></span>
            </h5>
                            ...
                            <tr>
                                <td>
                                    <code style="font-size:clamp(11px,0.75vw,13px);color:var(--medical-blue);font-weight:600;background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;">
                                        <?= htmlspecialchars($r['id_region']) ?>
                                    </code>
                                </td>
                                <td><strong><?= htmlspecialchars($r['titre_region']) ?></strong></td>
                                <td><?= getStatusBadge($r['etat_region']) ?></td>
                                <td>
                                    <div class="actions-group">
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= $r['id_region'] ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= $r['id_region'] ?>" 
                                            data-nom="<?= htmlspecialchars($r['titre_region']) ?>" 
                                            title="Supprimer">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-globe me-2"></i>Nouvelle région</h5>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Région <span class="text-danger">*</span></label>
                            <div class="code-input-group">
                                <input type="text" class="form-control" id="add_id_region" name="sai_id_region" 
                                       value="<?= htmlspecialchars($generated_id) ?>" required maxlength="20">
                                <button type="button" id="generateIdBtn" class="btn-generate-id">
                                    <i class="bi bi-arrow-repeat"></i>
                                </button>
                            </div>
                            <small class="text-muted">Format: REG + Année + 4 chiffres</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom de la région <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_region" required maxlength="100" placeholder="Ex: Abidjan, Yamoussoukro...">
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État de la région <span class="text-danger">*</span></label>
                            <select name="sai_etat_region" class="form-select" required>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la région</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="sai_id_region" id="delete_id">
                    
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette région ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_region_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Nom :</span>
                            <span style="font-weight:500;" id="delete_region_nom"></span>
                        </div>
                    </div>
                    
                    <div class="alert-medical alert-info mt-3" style="font-size:12px;text-align:left;">
                        <i class="bi bi-exclamation-triangle me-1"></i>
                        <strong>Attention :</strong> Cette action est irréversible. Si la région est associée à des districts, la suppression sera bloquée.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
...
$(document).ready(function() {
    ...
    $('#generateIdBtn').click(function() {
        ...
        $.post(window.location.href, { ajax_action: 'generate_id' }, function(data) {
            if (data.success) {
                $('#add_id_region').val(data.id_region);
            } else {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de la région...</p>
            </div>
        `);
        
        $.post(window.location.href, { 
            ajax_action: 'load_region', 
            id: id 
        }, function(data) {
            if (data.success) {
                const r = data.data;
                body.html(`
                    <input type="hidden" name="sai_id_region" value="${r.id_region}">
                    
                    <div class="section-title"><i class="bi bi-hash"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Région</label>
                            <input type="text" class="form-control" value="${r.id_region}" readonly style="font-family:monospace;font-weight:600;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom de la région <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_region" value="${r.titre_region}" required maxlength="100">
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État de la région <span class="text-danger">*</span></label>
                            <select name="sai_etat_region" class="form-select" required>
                                <option value="ACTIF" ${r.etat_region === 'ACTIF' ? 'selected' : ''}>ACTIF</option>
                                <option value="INACTIF" ${r.etat_region === 'INACTIF' ? 'selected' : ''}>INACTIF</option>
                            </select>
                        </div>
                    </div>
                `);
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        $('#delete_id').val(id);
        $('#delete_region_id').text(id);
        $('#delete_region_nom').text(nom);
    });
