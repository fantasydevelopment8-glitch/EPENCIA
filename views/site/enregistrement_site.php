...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_GET['edit']) && !empty($_GET['edit'])) {
            $stmt = $pdo->prepare("SELECT s.*, d.titre_district FROM site s LEFT JOIN district d ON s.id_district = d.id_district WHERE s.id_site = ?");
        ...
        if (isset($_GET['supprimer']) && !empty($_GET['supprimer'])) {
            ...
            header('Location: recherche_site.php?success=supprime');
            exit;
        }
...
if (isset($_GET['edit']) && !empty($_GET['edit']) && !$edit_site) {
    $stmt = $pdo->prepare("SELECT s.*, d.titre_district FROM site s LEFT JOIN district d ON s.id_district = d.id_district WHERE s.id_site = ?");
    $stmt->execute([$_GET['edit']]);
    $edit_site = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
// === VALEURS PAR DÉFAUT ===
$id_site = $edit_site['id_site'] ?? $generated_id;
$titre_site = $edit_site['titre_site'] ?? '';
$id_district = $edit_site['id_district'] ?? '';
$etat_site = $edit_site['etat_site'] ?? 'ACTIF';
$is_edit = ($edit_site !== null);
?>
...
<head>
    ...
    <title><?= $is_edit ? 'Modifier' : 'Ajouter' ?> un Site</title>
    ...
    <style>
        ...
        .info-site { background: #e8f4fd; border-left: 4px solid #0a4d8c; padding: 12px 15px; border-radius: 6px; font-size: 0.85rem; }
        .info-site strong { display: block; margin-bottom: 5px; color: #0a4d8c; font-size: 0.8rem; text-transform: uppercase; }
...
<div class="container my-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0"><?= $is_edit ? '✏️ Modifier le site' : '📝 Ajouter un site' ?></h4>
            <div class="text-muted small"><?= $is_edit ? 'Modifier les informations du site' : 'Créer un nouveau site' ?></div>
        </div>
    </div>
    <hr>
            ...
            <div class="card p-4">
                <form method="POST" id="siteForm">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label required">ID Site</label>
                            <div class="code-input-group">
                                <input type="text" class="form-control" id="sai_id_site" name="sai_id_site" 
                                       value="<?= htmlspecialchars($id_site) ?>" required maxlength="20"
                                       <?= $is_edit ? 'readonly' : '' ?>>
                                <?php if (!$is_edit): ?>
                            ...
                            <small class="text-muted"><i class="bi bi-info-circle"></i> Format: SIT + Année + 4 chiffres</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label required">Nom du site</label>
                            <input type="text" class="form-control" id="sai_titre_site" name="sai_titre_site" 
                                   value="<?= htmlspecialchars($titre_site) ?>" required maxlength="100"
                                   placeholder="Ex: Site d'Abobo">
                        </div>
                        
                        <div class="col-12 mt-2">
                            <div class="section-title"><i class="bi bi-building me-2"></i>Localisation</div>
                        </div>
                        <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            <label class="form-label required">État du site</label>
                            <select name="sai_etat_site" id="sai_etat_site" class="form-select" required>
                                <option value="ACTIF" <?= ($etat_site == 'ACTIF') ? 'selected' : '' ?>>ACTIF</option>
                                <option value="INACTIF" <?= ($etat_site == 'INACTIF') ? 'selected' : '' ?>>INACTIF</option>
                            </select>
                            <small class="text-muted"><i class="bi bi-info-circle"></i> Un site inactif n'est pas disponible pour les données</small>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-center gap-2 mt-4">
                        <?php if ($is_edit): ?>
                            ...
                            <button type="submit" name="btn_supprimer" class="btn btn-danger-cbs px-4" 
                                    onclick="return confirm('⚠️ Supprimer ce site ? Cette action est irréversible.')">
                                <i class="bi bi-trash me-2"></i>Supprimer
                            </button>
                            <a href="enregistrement_site.php" class="btn btn-outline-cbs px-4">
            ...
            <div class="card p-4">
                ...
                <div class="mb-3">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <div>
                            <strong>Création de site</strong>
                            <p class="text-muted small mb-0">L'ID est généré automatiquement au format SIT + Année + 4 chiffres</p>
                        </div>
                    </div>
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <div>
                            <strong>District associé</strong>
                            <p class="text-muted small mb-0">Un site appartient à un district</p>
                        </div>
                    </div>
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <p class="text-muted small mb-0">Contrôle la disponibilité du site</p>
                        </div>
                    </div>
                </div>
                ...
                <?php if ($is_edit): ?>
                <div class="info-site">
                    <strong><i class="bi bi-building me-1"></i> District</strong>
                    <?= htmlspecialchars($edit_site['titre_district'] ?? 'Non défini') ?>
                </div>
                <div class="info-site mt-2">
                    <strong><i class="bi bi-shield me-1"></i> Statut actuel</strong>
                    <span class="badge-<?= strtolower($etat_site) ?>"><?= $etat_site ?></span>
                </div>
                <?php endif; ?>
                
                <div class="alert alert-warning small mt-3 mb-0">
                    <i class="bi bi-exclamation-triangle me-2"></i>
                    <strong>Suppression :</strong> Impossible si le site est utilisé dans des données.
                </div>
            </div>
        </div>
    </div>
