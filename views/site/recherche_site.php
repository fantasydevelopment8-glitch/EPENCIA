...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'generate_id') {
        $year = date('y');
        $stmt = $pdo->prepare("SELECT id_site FROM site WHERE id_site LIKE ? ORDER BY id_site DESC LIMIT 1");
        $stmt->execute(['SIT' . $year . '%']);
        $last = $stmt->fetchColumn();
        $num = $last ? str_pad(intval(substr($last, -4)) + 1, 4, '0', STR_PAD_LEFT) : '0001';
        $id_site = 'SIT' . $year . $num;
        echo json_encode(['success' => true, 'id_site' => $id_site]);
        exit;
    }
    
    if ($_POST['ajax_action'] === 'load_site') {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("SELECT s.*, d.titre_district, r.titre_region 
                               FROM site s 
                               LEFT JOIN district d ON s.id_district = d.id_district 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['btn_ajouter'])) {
            ...
            $titre_site = trim($_POST['sai_titre_site']);
            $id_district = trim($_POST['sai_id_district']);
            $etat_site = trim($_POST['sai_etat_site']);
            
            if (empty($id_site)) throw new Exception('L\'ID site est obligatoire.');
            if (empty($titre_site)) throw new Exception('Le titre site est obligatoire.');
            if (empty($id_district)) throw new Exception('Le district est obligatoire.');
            
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM site WHERE id_site = ?');
            $stmt->execute([$id_site]);
            if ($stmt->fetchColumn() > 0) throw new Exception('Cet ID de site est déjà utilisé.');
            
            $req = $pdo->prepare("INSERT INTO site (id_site, titre_site, id_district, etat_site) VALUES (?, ?, ?, ?)");
            $req->execute([$id_site, $titre_site, $id_district, $etat_site]);
            
            $message = "Site <strong>$titre_site</strong> créé avec succès !";
        }
        
        // MODIFIER
        if (isset($_POST['btn_modifier'])) {
            $id_site = $_POST['sai_id_site'];
            $titre_site = trim($_POST['sai_titre_site']);
            $id_district = trim($_POST['sai_id_district']);
            $etat_site = trim($_POST['sai_etat_site']);
            
            if (empty($id_site) || empty($titre_site) || empty($id_district)) {
                throw new Exception('Les champs obligatoires doivent être remplis.');
            }
            
            $req = $pdo->prepare("UPDATE site SET titre_site = ?, id_district = ?, etat_site = ? WHERE id_site = ?");
            $req->execute([$titre_site, $id_district, $etat_site, $id_site]);
        ...
        if (isset($_POST['btn_supprimer'])) {
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM donnee WHERE id_site = ?');
            $stmt->execute([$id_site]);
            if ($stmt->fetchColumn() > 0) {
                throw new Exception("Ce site est utilisé dans des données.");
            }
            
            $req = $pdo->prepare("DELETE FROM site WHERE id_site = ?");
            $req->execute([$id_site]);
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['site_filters']['search'] ?? '');
$filter_etat = trim($_POST['etat'] ?? $_SESSION['site_filters']['etat'] ?? '');
$filter_district = trim($_POST['district'] ?? $_SESSION['site_filters']['district'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['site_filters'] = [
        'search' => $search,
        'etat' => $filter_etat,
...
$districts = $pdo->query("SELECT d.*, r.titre_region FROM district d LEFT JOIN region r ON d.id_region = r.id_region ORDER BY d.titre_district")->fetchAll(PDO::FETCH_ASSOC);

// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM site s LEFT JOIN district d ON s.id_district = d.id_district';
$count_params = [];
$where_clauses = [];

if (!empty($search)) {
    $where_clauses[] = '(s.id_site LIKE ? OR s.titre_site LIKE ? OR d.titre_district LIKE ?)';
    $search_term = "%$search%";
    $count_params = array_merge($count_params, [$search_term, $search_term, $search_term]);
}
if (!empty($filter_etat)) {
    $where_clauses[] = 's.etat_site = ?';
    $count_params[] = $filter_etat;
}
if (!empty($filter_district)) {
    $where_clauses[] = 's.id_district = ?';
...
$total_pages = max(1, ceil($total_sites / $limit));

// RÉCUPÉRER LES SITES
$sql = 'SELECT s.*, d.titre_district, r.titre_region 
        FROM site s 
        LEFT JOIN district d ON s.id_district = d.id_district 
        LEFT JOIN region r ON d.id_region = r.id_region';
$params = [];
...
$sql .= ' ORDER BY s.titre_site ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM site");
$total = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM site WHERE etat_site = 'ACTIF'");
$actifs = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM site WHERE etat_site = 'INACTIF'");
$inactifs = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(DISTINCT id_district) FROM site");
$districts_couverts = $stmt->fetchColumn();
...
<head>
    ...
    <title>Gestion des Sites - Epencia SGI</title>
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">sites</span></h1>
            <div class="subtitle">
                <i class="bi bi-geo-alt"></i>
                Consulter, ajouter, modifier et supprimer les sites
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouveau site</span>
            </button>
        </div>
    </header>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon info"><i class="bi bi-geo-alt"></i></div>
            <div class="stat-content">
        ...
        <div class="stat-card fade-in fade-in-d4">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= number_format($districts_couverts) ?></div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID, nom du site, district...">
            </div>
            <div class="col-md-3">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des sites
                <span class="badge bg-secondary ms-2"><?= number_format($total_sites) ?></span>
            </h5>
                            ...
                            <tr>
                                <td>
                                    <code style="font-size:clamp(11px,0.75vw,13px);color:var(--medical-blue);font-weight:600;background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;">
                                        <?= htmlspecialchars($s['id_site']) ?>
                                    </code>
                                </td>
                                <td><strong><?= htmlspecialchars($s['titre_site']) ?></strong></td>
                                <td><?= htmlspecialchars($s['titre_district'] ?? '-') ?></td>
                                <td><?= htmlspecialchars($s['titre_region'] ?? '-') ?></td>
                                <td><?= getStatusBadge($s['etat_site']) ?></td>
                                <td>
                                    <div class="actions-group">
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= $s['id_site'] ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= $s['id_site'] ?>" 
                                            data-nom="<?= htmlspecialchars($s['titre_site']) ?>" 
                                            title="Supprimer">
                    ...
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <form method="POST" class="d-inline page-form">
                                ...
                                <input type="hidden" name="district" value="<?= htmlspecialchars($filter_district) ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-geo-alt me-2"></i>Nouveau site</h5>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Site <span class="text-danger">*</span></label>
                            <div class="code-input-group">
                                <input type="text" class="form-control" id="add_id_site" name="sai_id_site" 
                                       value="<?= htmlspecialchars($generated_id) ?>" required maxlength="20">
                                <button type="button" id="generateIdBtn" class="btn-generate-id">
                                    <i class="bi bi-arrow-repeat"></i>
                                </button>
                            </div>
                            <small class="text-muted">Format: SIT + Année + 4 chiffres</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom du site <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_site" required maxlength="100" placeholder="Ex: Site d'Abobo">
                        </div>
                    </div>

                    <div class="section-title"><i class="bi bi-building"></i> Localisation</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            <label class="form-label">District <span class="text-danger">*</span></label>
                            <select name="sai_id_district" id="add_id_district" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un district --</option>
                                <?php foreach ($districts as $d): ?>
                                    <option value="<?= $d['id_district'] ?>">
                                        <?= htmlspecialchars($d['titre_district']) ?>
                                        <?php if (!empty($d['titre_region'])): ?>
                                            (<?= htmlspecialchars($d['titre_region']) ?>)
                                        <?php endif; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État du site <span class="text-danger">*</span></label>
                            <select name="sai_etat_site" class="form-select" required>
                                <option value="ACTIF" selected>ACTIF</option>
                                <option value="INACTIF">INACTIF</option>
                            </select>
                            <small class="text-muted">Un site inactif n'est pas disponible pour les données</small>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier le site</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="sai_id_site" id="delete_id">
                    
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer ce site ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_site_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Nom :</span>
                            <span style="font-weight:500;" id="delete_site_nom"></span>
                        </div>
                    </div>
                    
                    <div class="alert-medical alert-info mt-3" style="font-size:12px;text-align:left;">
                        <i class="bi bi-exclamation-triangle me-1"></i>
                        <strong>Attention :</strong> Cette action est irréversible. Si le site est utilisé dans des données, la suppression sera bloquée.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
...
$(document).ready(function() {
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement du site...</p>
            </div>
        `);
        
        $.post(window.location.href, { 
            ajax_action: 'load_site', 
            id: id 
        }, function(data) {
            if (data.success) {
                const s = data.data;
                body.html(`
                    <input type="hidden" name="sai_id_site" value="${s.id_site}">
                    
                    <div class="section-title"><i class="bi bi-hash"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Site</label>
                            <input type="text" class="form-control" value="${s.id_site}" readonly style="font-family:monospace;font-weight:600;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nom du site <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_site" value="${s.titre_site}" required maxlength="100">
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            <label class="form-label">District <span class="text-danger">*</span></label>
                            <select name="sai_id_district" id="edit_id_district" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez un district --</option>
                                <?php foreach ($districts as $d): ?>
                                    <option value="<?= $d['id_district'] ?>" ${s.id_district === '<?= $d['id_district'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($d['titre_district']) ?>
                                        <?php if (!empty($d['titre_region'])): ?>
                                            (<?= htmlspecialchars($d['titre_region']) ?>)
                                        <?php endif; ?>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État du site <span class="text-danger">*</span></label>
                            <select name="sai_etat_site" class="form-select" required>
                                <option value="ACTIF" ${s.etat_site === 'ACTIF' ? 'selected' : ''}>ACTIF</option>
                                <option value="INACTIF" ${s.etat_site === 'INACTIF' ? 'selected' : ''}>INACTIF</option>
                            </select>
                            <small class="text-muted">Un site inactif n'est pas disponible pour les données</small>
                        </div>
                    </div>
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        $('#delete_id').val(id);
        $('#delete_site_id').text(id);
        $('#delete_site_nom').text(nom);
    });
