<?php
// ================================================================
// GESTION DES TRANCHES D'ÂGE - EPENCIA SGI
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_tranche') {
        $id = $_POST['id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['btn_ajouter'])) {
            ...
            if ($age_debut === '' || $age_debut === null) throw new Exception('L\'âge début est obligatoire.');
            if ($age_fin === '' || $age_fin === null) throw new Exception('L\'âge fin est obligatoire.');
        ...
        if (isset($_POST['btn_modifier'])) {
            ...
            if (empty($id_tranche) || $age_debut === '' || $age_fin === '' || empty($titre_debut) || empty($titre_fin)) {
        ...
        if (isset($_POST['reset_filters'])) {
            unset($_SESSION['tranche_filters']);
        }
...
$offset = ($page - 1) * $limit;
$search = trim($_POST['search'] ?? $_SESSION['tranche_filters']['search'] ?? '');
$filter_etat = trim($_POST['etat'] ?? $_SESSION['tranche_filters']['etat'] ?? '');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['tranche_filters'] = [
        'search' => $search,
        'etat' => $filter_etat
    ];
}
...
// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM tranche_age';
$count_params = [];
$where_clauses = [];

if (!empty($search)) {
    $where_clauses[] = '(id_tranche LIKE ? OR titre_debut LIKE ? OR titre_fin LIKE ?)';
    $search_term = "%$search%";
    $count_params = array_merge($count_params, [$search_term, $search_term, $search_term]);
}
if (!empty($filter_etat)) {
    $where_clauses[] = 'etat_tranche_age = ?';
...
$total_pages = max(1, ceil($total_tranches / $limit));
...
$sql .= ' ORDER BY age_debut ASC LIMIT ? OFFSET ?';
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM tranche_age");
$total = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM tranche_age WHERE etat_tranche_age = 'ACTIF'");
$actifs = $stmt->fetchColumn();
$stmt = $pdo->query("SELECT COUNT(*) FROM tranche_age WHERE etat_tranche_age = 'INACTIF'");
$inactifs = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT *, (age_fin - age_debut) as ecart FROM tranche_age ORDER BY ecart DESC LIMIT 1");
$tranche_plus_large = $stmt->fetch(PDO::FETCH_ASSOC);

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_tranche) FROM projet_tranche");
$tranches_associees = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT id_tranche) FROM donnee");
$tranches_utilisees = $stmt->fetchColumn();
...
<head>
    ...
    <title>Gestion des Tranches d'Âge - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ============================================================ */
        /* RESPONSIVE - RÉDUCTION DES BOUTONS EN MOBILE                */
        /* ============================================================ */
        ...
        @media (max-width: 820px) {
            ...
            .modal-footer .btn-medical-primary,
            .modal-footer .btn-medical-secondary,
            .modal-footer .btn-medical-danger,
            .modal-footer .btn-medical-outline {
        ...
        @media (max-width: 576px) {
            ...
            .filter-section .form-control,
            .filter-section .form-select { height: 36px; font-size: 0.8rem; padding: 6px 10px; }
            ...
            .header-actions .btn-medical-primary,
            .header-actions .btn-back-adaptive {
                width: 100% !important;
            ...
            .header-actions .btn-medical-primary span {
                display: none !important;
            }
            .header-actions .btn-back-adaptive .btn-label {
                display: none !important;
            }
            .btn-back-adaptive {
            ...
            .btn-medical-primary i,
            .btn-medical-secondary i,
            .btn-medical-danger i,
            .btn-medical-outline i {
                font-size: 0.75rem !important;
            }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">tranches d'âge</span></h1>
            <div class="subtitle">
                <i class="bi bi-person"></i>
                Consulter, ajouter, modifier et supprimer les tranches d'âge
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouvelle tranche</span>
            </button>
        </div>
    </header>
    ...
    <div class="stats-grid">
        <div class="stat-card fade-in fade-in-d1">
            <div class="stat-icon info"><i class="bi bi-person"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($total) ?></div>
                <div class="stat-label">Total tranches</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d2">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Tranches actives</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d3">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">Tranches inactives</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d4">
            <div class="stat-icon blue"><i class="bi bi-rulers"></i></div>
            <div class="stat-content">
                <div class="stat-value">
                    <?php if ($tranche_plus_large): ?>
                        <?= $tranche_plus_large['age_debut'] ?>-<?= $tranche_plus_large['age_fin'] ?> ans
                    <?php else: ?>
                        0
                    <?php endif; ?>
                </div>
                <div class="stat-label">Plus large tranche</div>
                <div class="stat-sub"><?= $tranches_associees ?> associées • <?= $tranches_utilisees ?> utilisées</div>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-7">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ID, libellé début, libellé fin...">
            </div>
            <div class="col-md-3">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des tranches d'âge
                <span class="badge bg-secondary ms-2"><?= number_format($total_tranches) ?></span>
            </h5>
                ...
                <table class="table table-dashboard">
                    <thead>
                        <tr>
                            ...
                            <th style="width:100px;">Âge début</th>
                            <th style="width:100px;">Âge fin</th>
                            <th>Libellé début</th>
                            <th>Libellé fin</th>
                            ...
                            <tr>
                                <td>
                                    <code style="font-size:clamp(11px,0.75vw,13px);color:var(--medical-blue);font-weight:600;background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;">
                                        <?= htmlspecialchars($t['id_tranche']) ?>
                                    </code>
                                </td>
                                <td><span class="badge bg-primary"><?= $t['age_debut'] ?> ans</span></td>
                                <td><span class="badge bg-info"><?= $t['age_fin'] ?> ans</span></td>
                                <td><strong><?= htmlspecialchars($t['titre_debut']) ?></strong></td>
                                <td><strong><?= htmlspecialchars($t['titre_fin']) ?></strong></td>
                                <td><?= getStatusBadge($t['etat_tranche_age']) ?></td>
                                <td>
                                    <div class="actions-group">
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= $t['id_tranche'] ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= $t['id_tranche'] ?>" 
                                            data-nom="<?= htmlspecialchars($t['titre_debut'] . ' - ' . $t['titre_fin']) ?>" 
                                            title="Supprimer">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-person me-2"></i>Nouvelle tranche d'âge</h5>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Tranche <span class="text-danger">*</span></label>
                            <div class="code-input-group">
                                <input type="text" class="form-control" id="add_id_tranche" name="sai_id_tranche" 
                                       value="<?= htmlspecialchars($generated_id) ?>" required maxlength="20">
                                <button type="button" id="generateIdBtn" class="btn-generate-id">
                                    <i class="bi bi-arrow-repeat"></i>
                                </button>
                            </div>
                            <small class="text-muted">Format: TRA + Année + 4 chiffres</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Tranche d'âge <span class="text-danger">*</span></label>
                            <div class="row g-2">
                                <div class="col-6">
                                    <input type="number" class="form-control" name="sai_age_debut" required placeholder="Début" min="0" max="150">
                                </div>
                                <div class="col-6">
                                    <input type="number" class="form-control" name="sai_age_fin" required placeholder="Fin" min="0" max="150">
                                </div>
                            </div>
                            <small class="text-muted">Âge début doit être ≤ Âge fin</small>
                        </div>
                    </div>

                    <div class="section-title"><i class="bi bi-tag"></i> Libellés</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Libellé Début <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_debut" required maxlength="50" placeholder="Ex: Enfant, Jeune, Adulte...">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Libellé Fin <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_fin" required maxlength="50" placeholder="Ex: Enfant, Jeune, Adulte...">
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État de la tranche <span class="text-danger">*</span></label>
                            <select name="sai_etat_tranche_age" class="form-select" required>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la tranche d'âge</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="sai_id_tranche" id="delete_id">
                    
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette tranche d'âge ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <code style="font-size:13px;color:var(--medical-blue);background:var(--medical-blue-light);padding:2px 8px;border-radius:4px;" id="delete_tranche_id"></code>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Libellé :</span>
                            <span style="font-weight:500;" id="delete_tranche_nom"></span>
                        </div>
                    </div>
                    
                    <div class="alert-medical alert-info mt-3" style="font-size:12px;text-align:left;">
                        <i class="bi bi-exclamation-triangle me-1"></i>
                        <strong>Attention :</strong> Cette action est irréversible. Si la tranche est associée à des projets ou utilisée dans des données, la suppression sera bloquée.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
...
$(document).ready(function() {
    ...
    $('#generateIdBtn').click(function() {
        ...
        $.post(window.location.href, { ajax_action: 'generate_id' }, function(data) {
            if (data.success) {
                $('#add_id_tranche').val(data.id_tranche);
            } else {
    ...
    // ============================================================
    $('input[name="sai_age_debut"], input[name="sai_age_fin"]').on('change', function() {
        const debut = parseInt($('input[name="sai_age_debut"]').val());
        const fin = parseInt($('input[name="sai_age_fin"]').val());
        if (!isNaN(debut) && !isNaN(fin) && debut > fin) {
            alert('L\'âge début doit être inférieur ou égal à l\'âge fin.');
            $('input[name="sai_age_fin"]').val(debut);
        }
    });
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        body.html(`
            <div class="text-center py-4">
                <div class="loading-spinner"></div>
                <p class="mt-2 text-muted">Chargement de la tranche d'âge...</p>
            </div>
        `);
        
        $.post(window.location.href, { 
            ajax_action: 'load_tranche', 
            id: id 
        }, function(data) {
            if (data.success) {
                const t = data.data;
                body.html(`
                    <input type="hidden" name="sai_id_tranche" value="${t.id_tranche}">
                    
                    <div class="section-title"><i class="bi bi-hash"></i> Identification</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">ID Tranche</label>
                            <input type="text" class="form-control" value="${t.id_tranche}" readonly style="font-family:monospace;font-weight:600;">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Tranche d'âge <span class="text-danger">*</span></label>
                            <div class="row g-2">
                                <div class="col-6">
                                    <input type="number" class="form-control" name="sai_age_debut" value="${t.age_debut}" required min="0" max="150">
                                </div>
                                <div class="col-6">
                                    <input type="number" class="form-control" name="sai_age_fin" value="${t.age_fin}" required min="0" max="150">
                                </div>
                            </div>
                            <small class="text-muted">Âge début doit être ≤ Âge fin</small>
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Libellé Début <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_debut" value="${t.titre_debut}" required maxlength="50">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Libellé Fin <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="sai_titre_fin" value="${t.titre_fin}" required maxlength="50">
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">État de la tranche <span class="text-danger">*</span></label>
                            <select name="sai_etat_tranche_age" class="form-select" required>
                                <option value="ACTIF" ${t.etat_tranche_age === 'ACTIF' ? 'selected' : ''}>ACTIF</option>
                                <option value="INACTIF" ${t.etat_tranche_age === 'INACTIF' ? 'selected' : ''}>INACTIF</option>
                            </select>
                        </div>
                    </div>
                `);
                ...
                body.html(`
                    <div class="alert-medical alert-danger">
                        ...
                        Erreur lors du chargement de la tranche d'âge.
                    </div>
                `);
            }
        }, 'json');
    ...
    $('#deleteModal').on('show.bs.modal', function(e) {
        ...
        $('#delete_id').val(id);
        $('#delete_tranche_id').text(id);
        $('#delete_tranche_nom').text(nom);
    });
