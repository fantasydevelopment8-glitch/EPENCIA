...
function getLectureBadge($lecture) {
    if ($lecture == 1) {
        return '<span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>Lu</span>';
    } else {
        return '<span class="badge bg-danger"><i class="bi bi-circle me-1"></i>Non lu</span>';
    }
}

function getAffichageBadge($affichage) {
    if ($affichage == 1) {
        return '<span class="badge bg-primary"><i class="bi bi-eye me-1"></i>Visible</span>';
    } else {
        return '<span class="badge bg-secondary"><i class="bi bi-eye-slash me-1"></i>Masqué</span>';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'load_vue') {
        $id = (int)$_POST['id'];
        $stmt = $pdo->prepare("SELECT v.*, n.titre as notification_titre, n.objet as notification_objet,
                               u.nom_prenom as utilisateur_nom
                               FROM vue v 
                               LEFT JOIN notifications n ON v.notification = n.id 
                               LEFT JOIN utilisateurs u ON v.user = u.utilisateur_id 
                               WHERE v.id = ?");
        $stmt->execute([$id]);
        ...
        if ($data) {
            ...
            echo json_encode(['success' => false, 'message' => 'Vue introuvable']);
        }
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['btn_ajouter'])) {
            $notification = (int)$_POST['sai_notification'];
            $user = trim($_POST['sai_user']);
            $lecture = isset($_POST['sai_lecture']) ? (int)$_POST['sai_lecture'] : 0;
            $affichage = isset($_POST['sai_affichage']) ? (int)$_POST['sai_affichage'] : 1;
        ...
        if (isset($_POST['btn_modifier'])) {
            ...
            if (empty($id) || empty($notification) || empty($user)) {
            ...
            if ($stmt->fetchColumn() > 0) {
                throw new Exception('Cette entrée existe déjà pour cet utilisateur et cette notification.');
            }
        ...
        // MARQUER COMME LU
        if (isset($_POST['marquer_lu']) && !empty($_POST['marquer_lu'])) {
...
$filter_user = trim($_POST['user'] ?? $_GET['user'] ?? '');
$filter_lecture = isset($_POST['lecture']) ? trim($_POST['lecture']) : (isset($_GET['lecture']) ? trim($_GET['lecture']) : (isset($_SESSION['vue_filters']['lecture']) ? $_SESSION['vue_filters']['lecture'] : ''));
$filter_affichage = isset($_POST['affichage']) ? trim($_POST['affichage']) : (isset($_GET['affichage']) ? trim($_GET['affichage']) : (isset($_SESSION['vue_filters']['affichage']) ? $_SESSION['vue_filters']['affichage'] : ''));

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_filters'])) {
    $_SESSION['vue_filters'] = [
        'search' => $search,
        'user' => $filter_user,
        'lecture' => $filter_lecture,
        'affichage' => $filter_affichage
    ];
} elseif (isset($_SESSION['vue_filters']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $search = $_SESSION['vue_filters']['search'] ?? '';
    $filter_user = $_SESSION['vue_filters']['user'] ?? '';
    $filter_lecture = $_SESSION['vue_filters']['lecture'] ?? '';
    $filter_affichage = $_SESSION['vue_filters']['affichage'] ?? '';
    $page = $_SESSION['vue_filters']['page'] ?? 1;
    $offset = ($page - 1) * $limit;
}
...
$notifications = $pdo->query("SELECT id, titre, objet FROM notifications ORDER BY date DESC")->fetchAll(PDO::FETCH_ASSOC);

// COMPTER TOTAL
$count_sql = 'SELECT COUNT(*) FROM vue v 
              LEFT JOIN notifications n ON v.notification = n.id 
              LEFT JOIN utilisateurs u ON v.user = u.utilisateur_id';
$count_params = [];
$where_clauses = [];
...
if (!empty($search)) {
    $where_clauses[] = '(n.titre LIKE ? OR n.objet LIKE ? OR u.nom_prenom LIKE ?)';
    $search_term = "%$search%";
    $count_params = array_merge($count_params, [$search_term, $search_term, $search_term]);
}
if (!empty($filter_user)) {
    $where_clauses[] = 'v.user = ?';
    $count_params[] = $filter_user;
}
if ($filter_lecture !== '' && $filter_lecture !== null) {
    $where_clauses[] = 'v.lecture = ?';
    $count_params[] = (int)$filter_lecture;
}
if ($filter_affichage !== '' && $filter_affichage !== null) {
    $where_clauses[] = 'v.affichage = ?';
    $count_params[] = (int)$filter_affichage;
}
...
$total_vues = $stmt->fetchColumn();
$total_pages = max(1, ceil($total_vues / $limit));

// RÉCUPÉRER LES VUES
$sql = 'SELECT v.id, v.notification, v.user, v.lecture, v.affichage, 
               n.titre as notification_titre, n.objet as notification_objet, n.date as notification_date,
               u.nom_prenom as utilisateur_nom, u.utilisateur_id
        FROM vue v 
        LEFT JOIN notifications n ON v.notification = n.id 
        LEFT JOIN utilisateurs u ON v.user = u.utilisateur_id';
$params = [];
...
if ($filter_lecture !== '' && $filter_lecture !== null) {
    $where_clauses[] = 'v.lecture = ?';
    $params[] = (int)$filter_lecture;
}
if ($filter_affichage !== '' && $filter_affichage !== null) {
    $where_clauses[] = 'v.affichage = ?';
    $params[] = (int)$filter_affichage;
}
if (!empty($where_clauses)) {
    $sql .= ' WHERE ' . implode(' AND ', $where_clauses);
}
$sql .= ' ORDER BY v.id DESC LIMIT ' . (int)$limit . ' OFFSET ' . (int)$offset;
...
// STATISTIQUES
$stmt = $pdo->query("SELECT COUNT(*) FROM vue");
$total = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM vue WHERE lecture = 1");
$lus = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM vue WHERE lecture = 0");
$non_lus = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM vue WHERE affichage = 1");
$visibles = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(*) FROM vue WHERE affichage = 0");
$masques = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT user) FROM vue");
$utilisateurs_concernes = $stmt->fetchColumn();
...
if (!empty($filter_user)) {
    ...
    foreach ($utilisateurs as $u) {
        if ($u['utilisateur_id'] == $filter_user) {
            $user_label = $u['nom_prenom'];
            break;
        }
    }
    $active_filters[] = ['label' => 'Utilisateur', 'value' => $user_label ?: $filter_user];
}
if ($filter_lecture !== '' && $filter_lecture !== null) {
    $active_filters[] = ['label' => 'Lecture', 'value' => $filter_lecture == 1 ? 'Lu' : 'Non lu'];
}
if ($filter_affichage !== '' && $filter_affichage !== null) {
    $active_filters[] = ['label' => 'Affichage', 'value' => $filter_affichage == 1 ? 'Visible' : 'Masqué'];
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Gestion des Vues - Epencia SGI</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .progress-section .progress-label h6 {
            font-weight: 700;
            color: var(--medical-text);
            margin: 0;
        }
        ...
        .btn-action.read:hover { background: var(--medical-teal); color: white; }
        .btn-action.read { background: var(--medical-teal-light); color: var(--medical-teal); }
        ...
        @media (max-width: 576px) {
            ...
            .progress-section .progress-label h6 { font-size: 13px; }
...
<div class="app-container">
    ...
    <header class="page-header">
        <div class="header-left">
            ...
            <h1>Gestion des <span class="highlight">vues</span></h1>
            <div class="subtitle">
                <i class="bi bi-eye"></i>
                Consulter et gérer les lectures de notifications
                <span class="dot"></span>
        ...
        <div class="header-actions">
            <button class="btn-medical-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="bi bi-plus-circle"></i> <span>Nouvelle vue</span>
            </button>
        </div>
    </header>
    ...
    <div class="stats-grid">
        <div class="stat-card fade-in fade-in-d1">
            <div class="stat-icon info"><i class="bi bi-eye"></i></div>
        ...
        <div class="stat-card fade-in fade-in-d2">
            ...
            <div class="stat-content">
                <div class="stat-value"><?= number_format($lus) ?></div>
                <div class="stat-label">Notifications lues</div>
                <div class="stat-sub"><?= $taux_lecture ?>% de lecture</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d3">
            <div class="stat-icon warning"><i class="bi bi-clock"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($non_lus) ?></div>
                <div class="stat-label">Non lues</div>
                <div class="stat-sub"><?= number_format($utilisateurs_concernes) ?> utilisateurs</div>
            </div>
        </div>
        <div class="stat-card fade-in fade-in-d4">
            <div class="stat-icon blue"><i class="bi bi-eye"></i></div>
            <div class="stat-content">
                <div class="stat-value"><?= number_format($visibles) ?></div>
                <div class="stat-label">Notifications visibles</div>
                <div class="stat-sub"><?= number_format($masques) ?> masquées</div>
            </div>
        </div>
    </div>
    ...
    <div class="progress-section fade-in">
        <div class="progress-label">
            <div>
                <h6><i class="bi bi-graph-up-arrow me-2" style="color:var(--medical-blue);"></i>Taux de lecture global</h6>
                <small class="text-muted"><?= number_format($lus) ?> notifications lues sur <?= number_format($total) ?> au total</small>
            </div>
            <div>
                <span class="badge bg-success" style="font-size:1.1rem;padding:6px 16px;"><?= $taux_lecture ?>%</span>
            </div>
        </div>
        <div class="progress">
            <div class="progress-bar" style="width: <?= $taux_lecture ?>%;"></div>
        </div>
    </div>
    ...
    <form method="POST" action="" id="filterForm" class="filter-section fade-in">
        ...
        <div class="row g-3 align-items-end">
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Titre, objet, utilisateur...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Utilisateur</label>
                <select name="user" id="filterUser" class="form-select">
                    <option value="">Tous</option>
                    <?php foreach ($utilisateurs as $u): ?>
                        <option value="<?= $u['utilisateur_id'] ?>" <?= $filter_user == $u['utilisateur_id'] ? 'selected' : '' ?>>
            ...
            <div class="col-md-2">
                ...
                <select name="lecture" id="filterLecture" class="form-select">
                    <option value="">Tous</option>
                    <option value="1" <?= $filter_lecture === '1' ? 'selected' : '' ?>>Lu</option>
                    <option value="0" <?= $filter_lecture === '0' ? 'selected' : '' ?>>Non lu</option>
                </select>
            </div>
            <div class="col-md-2">
                <label class="form-label">Affichage</label>
                <select name="affichage" id="filterAffichage" class="form-select">
                    <option value="">Tous</option>
                    <option value="1" <?= $filter_affichage === '1' ? 'selected' : '' ?>>Visible</option>
                    <option value="0" <?= $filter_affichage === '0' ? 'selected' : '' ?>>Masqué</option>
                </select>
            </div>
            <div class="col-md-2">
    ...
    <div class="card-modern fade-in">
        <div class="card-header">
            <h5>
                ...
                Liste des vues
                <span class="badge bg-secondary ms-2"><?= number_format($total_vues) ?></span>
            </h5>
            <?php if (!empty($active_filters)): ?>
                ...
                <table class="table table-dashboard">
                    <thead>
                        <tr>
                            ...
                            <th style="width:100px;">Affichage</th>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5" style="color:var(--medical-text-muted);">
                                    ...
                                    <?php if ($total == 0): ?>
                                        ...
                                        <br>
                                        <span style="font-size:13px;">Cliquez sur "Nouvelle vue" pour en créer une.</span>
                                    <?php else: ?>
                                        <strong>Aucune vue trouvée avec ces filtres.</strong>
                                        <br>
                                        <span style="font-size:13px;">Essayez de modifier vos filtres de recherche.</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php else: foreach ($vues as $v): ?>
                            <tr>
                                <td>
                                    <span class="fw-bold" style="color:var(--medical-text-secondary);">#<?= $v['id'] ?></span>
                                </td>
                                <td><strong><?= htmlspecialchars($v['notification_titre'] ?? '-') ?></strong></td>
                                <td>
                                    <span class="badge bg-secondary"><?= htmlspecialchars($v['notification_objet'] ?? '-') ?></span>
                                </td>
                                <td><?= htmlspecialchars($v['utilisateur_nom'] ?? '-') ?></td>
                                <td><?= getLectureBadge($v['lecture'] ?? 0) ?></td>
                                <td><?= getAffichageBadge($v['affichage'] ?? 1) ?></td>
                                <td>
                                    <div class="actions-group">
                                        <?php if (($v['lecture'] ?? 0) == 0): ?>
                                            <form method="POST" class="d-inline">
                                                <input type="hidden" name="marquer_lu" value="<?= $v['id'] ?>">
                                        ...
                                        <button type="button" class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editModal" 
                                            data-id="<?= $v['id'] ?>" title="Modifier">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button type="button" class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                            data-id="<?= $v['id'] ?>" 
                                            data-nom="<?= htmlspecialchars($v['notification_titre'] ?? 'N/A') ?>" 
                                            data-user="<?= htmlspecialchars($v['utilisateur_nom'] ?? 'N/A') ?>" 
                                            title="Supprimer">
                ...
                <div class="d-flex justify-content-center p-3">
                    <ul class="pagination mb-0">
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&lecture=<?= urlencode($filter_lecture) ?>&affichage=<?= urlencode($filter_affichage) ?>">
                        ...
                        if ($start > 1): ?>
                            <li class="page-item">
                                <a class="page-link" href="?page=1&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&lecture=<?= urlencode($filter_lecture) ?>&affichage=<?= urlencode($filter_affichage) ?>">1</a>
                            </li>
                            <?php if ($start > 2): ?>
                        ...
                        <?php for ($i = $start; $i <= $end; $i++): ?>
                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&lecture=<?= urlencode($filter_lecture) ?>&affichage=<?= urlencode($filter_affichage) ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                        <?php if ($end < $total_pages): ?>
                            ...
                            <?php endif; ?>
                            <li class="page-item">
                                <a class="page-link" href="?page=<?= $total_pages ?>&search=<?= urlencode($search) ?>&user=<?= urlencode($filter_user) ?>&lecture=<?= urlencode($filter_lecture) ?>&affichage=<?= urlencode($filter_affichage) ?>"><?= $total_pages ?></a>
                            </li>
                        <?php endif; ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical success">
                <h5 class="modal-title"><i class="bi bi-eye me-2"></i>Nouvelle vue</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" id="addForm">
                <div class="modal-body">
                    <div class="section-title"><i class="bi bi-link"></i> Association</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Notification <span class="text-danger">*</span></label>
                            <select name="sai_notification" id="addNotification" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez --</option>
                                <?php foreach ($notifications as $n): ?>
                                    <option value="<?= $n['id'] ?>"><?= htmlspecialchars($n['titre']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Utilisateur <span class="text-danger">*</span></label>
                            <select name="sai_user" id="addUser" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez --</option>
                                <?php foreach ($utilisateurs as $u): ?>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="sai_lecture" class="form-select" required>
                                <option value="0">Non lu</option>
                                <option value="1">Lu</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Affichage <span class="text-danger">*</span></label>
                            <select name="sai_affichage" class="form-select" required>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header modal-header-medical">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Modifier la vue</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" id="editForm">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn-medical-outline" data-bs-dismiss="modal">Annuler</button>
                    <button type="submit" name="btn_modifier" class="btn-medical-primary" style="display:none;" id="editSubmitBtn">
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <p style="font-weight:500;color:var(--medical-text);">Voulez-vous vraiment supprimer cette vue ?</p>
                    ...
                    <div style="background:var(--medical-gray-light);padding:14px 18px;border-radius:var(--medical-radius-sm);border-left:4px solid var(--danger);margin-top:12px;">
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            ...
                            <span style="font-weight:600;color:var(--medical-text);">Notification :</span>
                            <span style="font-weight:500;" id="delete_nom"></span>
                            <span style="color:var(--medical-text-muted);">|</span>
                            <span style="font-weight:600;color:var(--medical-text);">Utilisateur :</span>
                            <span style="font-weight:500;" id="delete_user"></span>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#filterLecture').select2({
        theme: 'default',
        width: '100%',
        placeholder: 'Tous',
        allowClear: true
    });
    $('#filterAffichage').select2({
        theme: 'default',
        width: '100%',
        placeholder: 'Tous',
        allowClear: true
    ...
    $('#editModal').on('show.bs.modal', function(e) {
        ...
        $.post(window.location.href, { 
            ajax_action: 'load_vue', 
            id: id 
        }, function(data) {
            if (data.success) {
                const v = data.data;
                body.html(`
                    <input type="hidden" name="sai_id" value="${v.id}">
                    
                    <div class="section-title"><i class="bi bi-link"></i> Association</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Notification <span class="text-danger">*</span></label>
                            <select name="sai_notification" id="editNotification" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez --</option>
                                <?php foreach ($notifications as $n): ?>
                                    <option value="<?= $n['id'] ?>" ${v.notification == <?= $n['id'] ?> ? 'selected' : ''}>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Utilisateur <span class="text-danger">*</span></label>
                            <select name="sai_user" id="editUser" class="form-select" required style="width:100%;">
                                <option value="">-- Sélectionnez --</option>
                                <?php foreach ($utilisateurs as $u): ?>
                                    <option value="<?= $u['utilisateur_id'] ?>" ${v.user == '<?= $u['utilisateur_id'] ?>' ? 'selected' : ''}>
                                        <?= htmlspecialchars($u['nom_prenom']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Lecture <span class="text-danger">*</span></label>
                            <select name="sai_lecture" class="form-select" required>
                                <option value="0" ${v.lecture == 0 ? 'selected' : ''}>Non lu</option>
                                <option value="1" ${v.lecture == 1 ? 'selected' : ''}>Lu</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Affichage <span class="text-danger">*</span></label>
                            <select name="sai_affichage" class="form-select" required>
                                <option value="1" ${v.affichage == 1 ? 'selected' : ''}>Visible</option>
                                <option value="0" ${v.affichage == 0 ? 'selected' : ''}>Masqué</option>
                            </select>
                        </div>
                    </div>
                `);
                
                $('#editNotification').select2({
                    theme: 'default',
                    width: '100%',
                    placeholder: 'Sélectionnez une notification',
                    allowClear: true,
